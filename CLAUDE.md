...
**Why this is a rule and not a preference: no tool can tell the two apart, and one of them rots.** `A20a`/`A20b` pass OFI thresholds at 2.0/0.5 — neither the method default nor the shipped 1.6/0.625 — and that is **legitimate**: they are refactor-equivalence tests comparing two code paths on identical inputs, so any consistent value serves. `A6` pins `trendGate:=10.0` against a shipped 23.0 and that is **stale**. To a machine they are the same shape, which is why the JSON↔POCO drift guard (`WalkPocoVsJson`, fixtures **`A62a`–`A62g`**, commit `3a89093`) explicitly **cannot** cover this case and why it falls to review.
