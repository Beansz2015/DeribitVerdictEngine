...
3. **Load the `crypto-trading-context` skill** — it carries the writing style and a generic copy of the trader profile. **Then read `docs/trader-profile.md` in full** (RULED 2026-09-14, trader-directed: always read the trader profile at handover). ⚠ The file and the skill's copy have drifted; the file's own header lists the verified conflicts and which source wins until the trader re-syncs it.
