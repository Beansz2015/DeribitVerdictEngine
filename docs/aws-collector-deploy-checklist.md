...
| **AWS** | **`a19acc4d-465f-446a-ad6f-479e0c6cb9a9`** | **v69 — UNCHANGED.** Not a settings edge, not a scoring edge, not a dataset boundary | **2026-09-25 13:31:02** (first row; process started 13:29:40) | **The RR-1 gap-repair deploy** — commit `691ab0c` (fix `72f262e`) via `tools/ops/collector.ps1 deploy`, ACCEPTED (PID 31820, 3 rows, 58 s span). The repair hole walk now sorts by `TradeSeq`, so a same-burst timestamp inversion no longer makes a phantom hole ([`gap-repair-rr1-seq-order-spec.md`](gap-repair-rr1-seq-order-spec.md)). Read-back 13:32 UTC: header 124, rows `SettingsVersion` 69; `ws_health.log` `DOWN` 13:29:42 → `OK` 13:31:09 (fast; the 3.6 min on the previous deploy did not recur); startup repair pass `PASS_CLEAN` (4 holes, 697 rows committed). The liquidation probe was not touched and kept running. ⚠ **The fix's live proof is still owed:** at the next copy-back, check `repair_status.log` for any `trade_seq` range repaired on more than one pass under this instance |
