...
>
> **PLATEAU VALIDATION — DONE + PASSED (2026-06-24, regen on the 5275-row post-soak book; report `20260623_192312`).** The deferred §6/§7 acceptance is met. 3-min failure rates plateau within `{15,30,45}` on every cell:
> - **ASIA MEDIUM_LONG** (the flagged concern) = **24%→13%→13%** @15/30/45 (n=45) — flattens at 30m, vs the old "still declining at 19% @15m". Barrier decomp confirms it's window-expiry *resolving*, not stop-outs: success 34→39→39, adverse 0/0/0, expiry 11→6→6. **The trades were working; the old 15m ceiling just couldn't see them finish.**
> - **LONDON MEDIUM_SHORT** 37→33→33 (n=146), **LONDON STRONG_SHORT** 54→51→51 (n=37), **ASIA STRONG_SHORT** 89→84→84 (n=19) — all flatten by 30m. **No multiplier widening needed** (ASIA does NOT still decline at 45m).
> - **45m earns ZERO new successes on any cell** (success count flat 30m→45m everywhere; for LONDON MEDIUM_SHORT 45m only converts expiry→adverse, 9→20). So `{15,30,45}` is sufficient-to-slightly-generous — the *opposite* of the too-short failure mode. 45m now functions as the plateau-confirming ceiling; keep it (without it you can't prove 30m is the plateau). Optional future tightening only.
> - **Strategic payoff:** the recal didn't just fix a metric — it REVEALED genuinely strong 3-min tiers previously hidden as window-expiry artifacts. **ASIA MEDIUM_LONG = 13% fail / 87% success / zero stop-outs @30m** is the best cell in the book; **LONDON MEDIUM_SHORT 33% @30m (n=146)** is solid.
> - Also satisfies **G2's ≥300-row review** (NY×1 2698 / LONDON×3 1780 / ASIA×3 797; belowMin=0 as predicted for the all-post-v35 book). **Hold-window recal is fully closed.**
