...
**Sequence — INVESTIGATE NOW (pre-cutover); FIX timing is conditional.** *(Revised 2026-06-24 on the trader's point — supersedes an earlier "defer the whole thing to after P3".)* The investigation is analysis-only (correlate CONFIRMED's cross-category confirmation inputs against barrier outcomes on the existing CSV) and the data is already in the book, so run it **before the transport flip**, in parallel with P3 implementation (§4 + §6 harness — it does **not** block them). Why it must lead rather than trail: the escalation below can only fire if we know the answer *before* cutting over. Defer the investigation to after cutover and we risk discovering a pre-cutover need too late — once `transport=ws`, a scoring re-tune straddles transports, the exact contamination G2 exists to prevent. The investigation resolves the branch:
- **Display-only fix** (re-tune CONFIRMED's definition/thresholds — the *likely* case, since `VerdictContext` is display-only with zero scoring impact, `architecture.md`): ship anytime, lowest priority, fine to land after P3.
- **Scoring-gating fix** (CONFIRMED should suppress/boost the verdict, not merely relabel): a scoring change → approval-gated **and** a recalibration event → land it **before** the flip, on single-transport REST data (joins the G2 set).

**Trigger:** now — run the correlation pass, then write it up spec-first. Its conclusion gates only the flip-ordering, never P3 implementation.
