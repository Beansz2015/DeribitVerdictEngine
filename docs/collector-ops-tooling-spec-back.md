...
| ⭐ **That `fetch` or `deploy` actually work against a real instance** | Still never run. `status` succeeding proves `Invoke-RemotePs`'s send/poll/parse path end-to-end, which `fetch` and `deploy` also use — but neither has executed its own additional logic (S3 transport, the backup/place/hash-verify chain, `Wait-DeployGate`'s corrected expression) even once |
