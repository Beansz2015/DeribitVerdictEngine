...
> | **T3** | ⚠ **You write the fixtures too.** A misunderstanding of `ReadSide` propagates into the test that was supposed to catch it. **Fixture A60c below names the input that makes it fail** — build that input first, watch it fail, then make it pass |
>
...
| **A60c** | `EpisodeSec` measures and resets | ⚠ **The input that makes this fail: open an episode, advance `nowMs`, close it via `CloseEpisode()`, re-open at the SAME level, read again.** A build that stores `EpisodeStartMs` but forgets the reset returns the FIRST episode's elapsed time and looks correct in every simpler test |
...
| 4 | ⭐ **One live run, then read the new columns in the row.** Confirm they are empty when `AbsorptionSignal` is `NONE` and populated when it is not |
