...
| # | Item | State |
|---|---|---|
| **8a** | Build gaps A and B into `FP-Q3`; re-run and re-measure | ✅ **BUILT 2026-09-22 (UTC)** — see §8.5. All six parameters now resolve a cfg path **by construction**. ✅ **RE-MEASURED 2026-09-22 (UTC) by the seat** — [`harness-runs/fixture-parser-a23a-run-2026-09-22.md`](harness-runs/fixture-parser-a23a-run-2026-09-22.md), baseline committed at `73a721c` before the run. **Four new `FP-1` sites, not six** (only marked sites join the residual). **5 of 5 agree, all STABLE** — but the four share one explicit declaration, so it is `n=1` on the deciding arm. The run's real result is `8j` and `8k` below |
...
| **8e** | `MTF_TTL_SECONDS` is `Private Const`; `CLAUDE.md` rules it `Public Const` so the fixture reads it instead of restating `60` | ⛔ **STOPPED — do not force it.** `MTF_TTL_SECONDS` lives in `UI/MainForm_Layout.vb`, a `MainForm` partial class, and `verify/ordercheck/OrderCheck.vbproj` links **no** `UI/*.vb` file by design. `Public Const` alone does **not** let the fixture read it; compiling `MainForm` into the fixture project would drag in `System.Windows.Forms` and break the host-agnostic boundary that vbproj asserts in six separate comments. ⭐ **The move that WOULD work, unbuilt and unruled: put the constant on the host-agnostic `MtfRefreshPolicy` (already linked) as `Public Const` and have `MainForm` read it there.** That edits a live-path file for a fixture's benefit, so it is the trader's call, not an implementer's. ✅ **RULED AND BUILT 2026-09-22 (UTC), trader: "agreed, go".** `MtfRefreshPolicy.TtlSeconds` is the one home (`Public Const` = 60); `MainForm_Layout.vb`'s `MTF_TTL_SECONDS` aliases it; `A16d`/`A16e` read it, and `A16e`'s probe ages are placed at half / exactly / one-and-a-half TTL so the boundary moves with the constant. Same value, so no behaviour change. Mutation: `TtlSeconds = 90` → 425 PASS (the fixtures follow the constant) |
| **8f** | ⛔ **NEW.** A **constructor** callee with an `Optional` parameter production never passes is unresolvable: `New WsMarketDataSource(…, staleAfterSec:=10)` reads `NO_SIGNATURE_FOUND` (the declaration is `Sub New`, not `Sub WsMarketDataSource`), and even with the signature, production **omits** the argument — the real key is read inside the constructor body (`SettingsLoader.Current.Network.WsStaleAfterSec`, `WsMarketDataSource.vb:37-38`). **A fourth shape — "the callee's own default-fallback body" — would close it** | ✅ **BUILT 2026-09-22 (UTC) at `ed3e487`** as `FP-D21` (revision 3): constructor signatures from the class's single `Sub New`, and the callee body's default-fallback expression when production omits the argument. `staleAfterSec` → `network.ws_stale_after_sec` [`DEFAULT_FALLBACK_BODY`]; nothing else in the no-key output moved; mutation-checked. ⛔ **Its one newly-opened site was measured by the seat: a STABLE DISAGREEMENT** — [`harness-runs/fixture-parser-stale-run-2026-09-22.md`](harness-runs/fixture-parser-stale-run-2026-09-22.md). See `8l` |
...
| **8h** | ⛔ **NEW.** `FP-Q1` answered `ttlSeconds` differently on two runs whose state for that parameter was **identical** (§8.5). `FP-D14` runs it unsampled, so nothing detects this | **Open. ⭐ CONFIRMED on seat review 2026-09-22 (UTC):** two further runs both excluded `ttlSeconds`, giving `IN_SCOPE_PARAMS=24` / `IN_SCOPE_SITES=57` against the build's 25 / 63 — **the delta is exactly that one parameter's six sites.** ⛔⛔ It was an **AGREEMENT** row in the measurement, so [`harness-shadow-mode-protocol.md`](harness-shadow-mode-protocol.md) §4d's *agreement ⇒ stable* does **not** carry across detectors; new §4f there. ✅ **BUILT 2026-09-22 (UTC) as `FP-D23`** (revision 3): 5 draws per parameter with a fresh `sample_uid`; **in scope if ANY draw says `threshold`** (the measured failure is under-scoping); `SCOPE_UNSTABLE_PARAMS` prints. Plus `FP-D24`: any residual item missing from the `FP-1` baseline stops the run (`BASELINE_INCOMPLETE`) before any `FP-1`/`FP-2` call, because a sampled scope can grow the residual mid-run. **First sampled reading, two keyed runs: 16 parameters × 5 draws each run, `SCOPE_UNSTABLE_PARAMS=0` both times, `ttlSeconds` `input` on all 10 draws; ≈ 60.6k input tokens ≈ $0.0025 and ~40 s a run.** ⚠ So the one observed flip has not recurred in 10 draws. That bounds it as rare; it does not show it is gone |
| **8i** | ⛔ **NEW, seat review 2026-09-22 (UTC). The `8d` declaration pushed `staleAfterSec` OUT of `FP-Q1` scope — `8g` is no longer hypothetical.** It was the detector's **only** hit in the measurement and the one site both judges agreed was in scope; after the MECHANISM comment it is excluded on both review runs, so **`FP-1` will never judge the declaration just written for it.** ⚠ The comment is the plausible cause and is **NOT established** — `ttlSeconds` flips unprompted, so one sample cannot separate cause from noise. **Fixing `8f` would make it immune**, since a site with a derived `FP-Q3` key is in scope by construction and never reaches `FP-Q1` | ✅ **CLOSED 2026-09-22 (UTC) by `8f`.** `staleAfterSec` is now in scope by construction and never reaches `FP-Q1`. ⚠ `8g`'s circularity still applies to any site that has NO derived key |
| **8j** | ⛔⛔ **NEW, `8a` re-measure 2026-09-22 (UTC). In-scope sites with NO marker never reach `FP-1`, and no counter reports them.** The residual is `HasMarker -and InScope`. Measured code-only: **34** in-scope sites carry no `MECHANISM`/`SHIPPED` marker, and **11 of them equal an ever-shipped value** — `A20a` ×3, `A20b` ×6, `A23b` ×2. A run exits 0 with `FP1_BAD_VERDICTS=0` while they sit there. ⛔ `CLAUDE.md` cites `A20a`/`A20b` as the legitimate example, and their `2.0`/`0.5` were shipped from 2026-04-22 to v48. Detail: [`harness-runs/fixture-parser-a23a-run-2026-09-22.md`](harness-runs/fixture-parser-a23a-run-2026-09-22.md) §5.1 | ✅ **COUNTERS BUILT 2026-09-22 (UTC) at `ed3e487`** as `FP-D22`: `IN_SCOPE_UNMARKED_SITES=34` and `IN_SCOPE_UNMARKED_EQUALS_EVER_SHIPPED=11` print in the coverage block, the 11 listed by site. Code facts only. ✅ **DECLARED 2026-09-22 (UTC) at `44e4e15`** by a separate Opus 5.5 agent (trader-ruled: the author does not also baseline). All MECHANISM, each mutation-checked; `IN_SCOPE_UNMARKED_EQUALS_EVER_SHIPPED` 11 → 0. **Measured by the seat: 12 of 14 agree** — [`harness-runs/fixture-parser-a20-a23b-run-2026-09-22.md`](harness-runs/fixture-parser-a20-a23b-run-2026-09-22.md). ⚠ The 23 off-shipped unmarked sites (`fundingBoost`, `upgradeBonus`) are still undeclared |
| **8l** | ⛔⛔ **NEW, 2026-09-22 (UTC). `FP-1` is STABLE AND WRONG on `SeededWsSource#2088#staleAfterSec`**: `declared_but_contradicted` 5 of 5, `mean_top_prob` 0.526, against the seat's upheld `mechanism_declared_ok`. [`harness-shadow-mode-protocol.md`](harness-shadow-mode-protocol.md) §4d's *"zero confident-and-wrong rows"* does not hold as a law; new §4g there | **Open. Cause NOT named.** The `A23a` block also discusses equality and keys and passed at 0.67–0.82. A controlled probe on the two comment texts is the next step, if pursued. ⛔ **Do NOT edit the comment to satisfy the verdict** — it is correct. ⛔ **Now THREE rows:** `A23b#3164#tauFastSec` and `#tauNormSec` came back `shipped_declared_ok`, STABLE 5/5, on a block that says MECHANISM and *"NOT SHIPPED BEHAVIOUR"* ([`harness-runs/fixture-parser-a20-a23b-run-2026-09-22.md`](harness-runs/fixture-parser-a20-a23b-run-2026-09-22.md) §3). ⛔⛔ **2026-09-23: 18 more, 15 of them STABLE** — the `fundingBoost`/`upgradeBonus` window agreed on only **4 of 22** ([`harness-runs/fixture-parser-funding-run-2026-09-23.md`](harness-runs/fixture-parser-funding-run-2026-09-23.md)). A controlled probe **refuted** "the comments mention the shipped value" as the cause; see `8o` |
| **8n** | ⛔ **NEW 2026-09-23 (UTC). A web firewall in front of the API blocks some request TEXT** (403, HTML block page): the `A8` comment phrase *"and 4 + 7 = 11"* reads as an SQL tautology. 1 of 37 items. It aborted the whole run on its first item | ✅ **BUILT as `FP-D26`:** `Invoke-Jev` flags `WafBlocked`; the item becomes `WAF_BLOCKED` / `NOT_JUDGED`, the run continues, a blocked item exits 1. ✅ **Harnesses 1 and 2 fixed too, 2026-09-23/24 (UTC), the "arm harnesses 1 and 2" batch:** `rider-travel.ps1` and `commit-walker.ps1` (the latter for both its primary AND escalation calls) apply the same pattern — `WAF_BLOCKED` / `NOT_JUDGED`, run continues, exit 1. Verified synthetically (no keyed call) in both; see `docs/harnesses-1-2-arming-spec-back.md`. Same batch also added bounded retries to `Invoke-Jev` itself (finding 11) — transient (5xx / no response) only, never a 4xx — which harnesses 3 and 4 inherit unchanged (regression-checked: their baseline-refusal output is unchanged) |
| **8o** | ⭐ **NEW 2026-09-23 (UTC). `FP-1`'s question wording moves its verdict.** One added criteria sentence (*naming the key only to argue the literal is not derived from it is consistent with MECHANISM*) flipped `A43d#7740` 4/5 wrong → 5/5 right and `A40b#6870` 5/5 wrong → 2/5 right | **Open — a candidate, not a fix.** Changing the question breaks comparability with every earlier measurement, and 2 items are not a validation. Needs a fresh, baselined population. ⛔ Until then `FP-1`'s `declared_but_contradicted` is not evidence of a breach on its own. ✅ **ARMED 2026-09-23 (UTC) at `ca566c5` as `FP-D30`** (harness revision 4): `FP-1v2` is asked BESIDE version 1 in the same call. Its criteria differ by the probe's sentence alone, byte-identical to the probe script, appended to `mechanism_declared_ok` AND `declared_but_contradicted` as the probe did. **Version 1 alone drives the verdict, the exit code and the baseline comparison.** Version 2's verdict and agreement rate print per item, plus `FP1V2_ITEMS_SAME_PLURALITY_AS_V1` and `FP1V2_SAMPLES_SAME_AS_V1`. Proved offline by [`../tools/checks/selftest/fixture-parser-selftest.ps1`](../tools/checks/selftest/fixture-parser-selftest.ps1). **Not measured; the re-measure on a fresh population is still owed.** ⚠ Not verified: whether asking `verdict_v2` in the same call moves version 1's answers |
| **8m** | ⛔⛔ **NEW, 2026-09-22 (UTC). The harness scores a misread as a PASS.** `shipped_declared_ok` is an OK class (exit 0), but `CLAUDE.md`'s provenance rule says SHIPPED BEHAVIOUR *"must derive the value from cfg, never hardcode it"* — and every `FP-1` site is a hardcoded literal. So on an `FP-1` site that verdict is either a rule breach or a misread, never a pass. **The `A23b` run exited 0 with two misreads in it** | ✅ **BUILT 2026-09-22 (UTC), trader-agreed, as `FP-D25`:** `shipped_declared_ok` now counts in `FP1_BAD_VERDICTS` and prints as `FP1_SHIPPED_ON_LITERAL`. **Scoring only** — the question sent to Jev is unchanged, so every earlier measurement stays comparable. Verified on a routine re-run of the `A23b` window: exit **1**, `FP1_SHIPPED_ON_LITERAL=2`; both misreads reproduced on that independent run (0.616 / 0.436) |
| **8k** | ⛔ **NEW, same run. Positional literals are invisible.** `A23b` passes six positional literals that all equal ever-shipped values (`Fold` 5.0/120.0 · `Snapshot` 50.0/120.0 · `ClassifyAggressorBurst` 2.5/0.2). The parser reads named arguments only (§4.1 step 1) | **Open, unsized** beyond `A23b`. ⛔ The item-`8d` comment on `A23a` also says `A23b` passes the taus positionally; its burst loop passes them **by name** (line 3092) |
| **8p** | **NEW 2026-09-23 (UTC).** [`jev-harnesses-adversarial-review-2026-09-22.md`](jev-harnesses-adversarial-review-2026-09-22.md) finding 4: signatures were looked up by bare method name, and the first declaration in file order won | ✅ **DONE 2026-09-23 (UTC) at `ca566c5` as `FP-D27`** (revision 4). A same-named callee resolves by its receiver type, or reports `AMBIGUOUS_CALLEE` (counted, never guessed). Measured on `Program.vb`: exactly 3 sites changed class. `A5#1042#currentATR` and `A19c`/`A19d` `nowUtcMs` left `PARAM_NOT_IN_SIGNATURE`. They are now read against `DynamicNorms.Compute` and `LiveMicrostructureEvaluator.Evaluate`. 0 `AMBIGUOUS_CALLEE` on the real file; 2 on the synthetic self-test source |
| **8q** | **NEW 2026-09-23 (UTC).** The same review's finding 8: the `MECHANISM`/`SHIPPED` marker is block-level | ✅ **DONE 2026-09-23 (UTC) at `ca566c5` as `FP-D28`.** It is a LABEL, not a filter. Each marked site prints `SITE_NAMED` or `BLOCK_ONLY`. In scope: 50 / 8 — the review's "8 of 58" exactly. Every marked site is still judged |
| **8r** | **NEW 2026-09-23 (UTC).** The same review's finding 9: the settings walk skipped JSON arrays | ✅ **DONE 2026-09-23 (UTC) at `ca566c5` as `FP-D29`.** Arrays of objects are walked by their `name` field, e.g. `session_volume.sessions[name=ASIA].high_multiplier`, ever-shipped {0.8, 1, 1.1} in 75 of 87 revisions (cross-checked independently in Python). `ARRAY_UNKEYED=0` at HEAD. Head leaves 254 → 271, the 17 new ones all array-keyed. The cache is schema-tagged. ⚠ No real call site reaches an array key yet: a cfg expression indexes with `(`, which FP-Q3 does not follow |
| **8s** | **NEW 2026-09-23 (UTC).** §5 acceptance item 8 / `FP-D3`: the `FP-2` mutation test, never run before | ✅ **RUN 2026-09-23 (UTC)** — [`harness-runs/fixture-parser-fp2-mutation-2026-09-23.md`](harness-runs/fixture-parser-fp2-mutation-2026-09-23.md). **6 of 6 mutated names flagged**; 5 STABLE, 1 UNSTABLE (`A65c`, 3 of 5). The six renames were applied to a temp COPY of `Program.vb`, never to the file itself. That supersedes §5 item 8's *"restore via `git checkout`"*, which has wiped uncommitted work in this repo once. Hash and `git diff --stat` were checked before and after. ⚠ These mutations are the easiest positive there is; subtle overclaims are untested |
