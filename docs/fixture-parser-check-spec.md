...
| **8o** | ⭐ **NEW 2026-09-23 (UTC). `FP-1`'s question wording moves its verdict.** One added criteria sentence (*naming the key only to argue the literal is not derived from it is consistent with MECHANISM*) flipped `A43d#7740` 4/5 wrong → 5/5 right and `A40b#6870` 5/5 wrong → 2/5 right | **Open — a candidate, not a fix.** Changing the question breaks comparability with every earlier measurement, and 2 items are not a validation. Needs a fresh, baselined population. ⛔ Until then `FP-1`'s `declared_but_contradicted` is not evidence of a breach on its own |
