...
| ⚠ **D1 re-read — is AWS-only capture still right?** | trader — **measured, not speculative** | ⚠ **NEW 2026-08-07, from the first AWS store copy-back.** Over 2026-08-03 21:44 → 08-05 13:13 UTC the AWS store held **78.8 %** of known trades and the local `bin\Release` box held **98.5 %**; **16,459 trades existed only on the local box**, and AWS was **healthy throughout** (no `DOWN`/`DEGRADED`, 921 analysis rows/day). Loss is **scattered across 36 hours**, not a contiguous outage — the shape of per-connection WS delivery differences, which no single subscriber can rule out for itself. **D1 ruled capture AWS-only on the reasoning that a second box recreates a topology it rejected; that reasoning did not price a sole capturer being systematically incomplete.** Options: **(a)** keep AWS-only and accept ~20 % scattered loss as the cost of a single book · **(b)** restore local capture as a *second sampler* — the union is what made the book complete here, and the merge cost is one `sort -u` · **(c)** keep AWS-only but make S0 (§ below) a daily job so loss is at least *measured*. **My read: (c) at minimum, and (b) is cheap** — the objection to two boxes was duplicate rows, and the retention rule already ruled duplicates cheap. Evidence: [`aws-collector-deploy-checklist.md`](aws-collector-deploy-checklist.md) §4a |
