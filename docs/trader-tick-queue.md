...
| ⚠ **Local capture ON — one action outstanding** | the trader, on the box | ✅ **RULED 2026-08-07: option (b).** Not a decision any more — a **file action**. Delete `settings.local.json` from **both** `bin\Debug\net8.0-windows` and `bin\Release\net8.0-windows`. That is the whole change; `trade_store.enabled` was the only key either overlay carried, and the tracked base already ships `true`. ⚠ **Expect `+local` to disappear from the local title bar** — that marker was the old daily alarm and is now retired (`aws-collector-deploy-checklist.md` §3, rewritten). Decision text: `in-app-trade-store-capture-proposal.md` §7 row **D1-a** |
