...
- **D1-a is RULED AND DONE** (2026-08-07). Local capture is ON in `bin\Debug` as a temporary second sampler; `bin\Release` keeps its overlay and must not capture. Do not re-raise it as a decision, and **do not "tidy" the Release overlay away** — it is what stops the deploy source repopulating `backtest_data\`.
