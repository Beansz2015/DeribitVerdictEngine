...
| ~~**A2**~~ | ~~**`settings.local.json` overlay — D1–D6**~~ | ✅ **D1–D6 ALL TICKED 2026-08-01, all-as-recommended → BUILD-AUTHORIZED. Hand off now.** **§1 of the brief overrides §2.2/§2.4 of the spec, and the tick carries it** — `alerts.` rejected, `mtf_gate` named, allow-list by construction, A50j added | brief: [`settings-local-overlay-implementer-brief.md`](settings-local-overlay-implementer-brief.md) |
| **A3** | **v64 runtime test** — *push done, test outstanding* | **Now gated only on A2 shipping.** Landing sequence is §6 of the brief: place `settings.local.json` in `bin\Debug\net8.0-windows\` **before** the first post-overlay Debug build | the test gate |
