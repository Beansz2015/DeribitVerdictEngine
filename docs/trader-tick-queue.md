...
| # | Tick | Gate | Where |
|---|---|---|---|
| ~~**C1**~~ | ~~**Trade-store coverage report — D1–D7**~~ | ✅ **ALL 7 TICKED 2026-08-03 — BUILD-AUTHORIZED.** Both parts in scope (D5=(a)). **A49** is the fixture family (verified free, 0 uses). **No ⚠ dataset boundary, no new HARD CONSTRAINT (HC27 already fences `trade_store.`), HC28 stays free** — so it cannot consume the one-⚠-per-window slot and cannot confound the running D3 ASIA watch. Two rulings bind it: [J-B scoping](j-b-scoping-ruling-2026-08-02.md) (six classes; D7=(b) ruled out) and [weekday scope](weekday-scope-ruling-2026-08-03.md) (weekday hours only; **Part B stays unconditional**). **No data dependency left** — D4 confirmed at 300,000 ms on its existing weekday basis, REST-backfill task cancelled | brief: [`trade-store-coverage-report-implementer-brief.md`](trade-store-coverage-report-implementer-brief.md) |
...
| **Atomic writes: swap the partial primitive for a total one** — ⚠ NEW 2026-08-05, from the order-app seat | 5 sites, mechanical, but they include `SettingsLoader.Save` so it wants fixtures | **The framing is theirs and it is the useful part: a guard is what you OWE when the primitive is PARTIAL. Choosing a TOTAL primitive removes the obligation rather than satisfying it.** `File.Replace` is partial — it throws on a missing destination — so all five of our sites need an existence guard, four have one, and `OhlcCache.vb:137` is safe only via a distant early-out. `File.Move(src, dst, overwrite:=True)` is **total**: it works whether or not the destination exists, is atomic on the same volume (MoveFileEx + MOVEFILE_REPLACE_EXISTING), and is available on .NET 8. Swapping it in **deletes the whole hazard class**, including the fragility we just had to write a comment about. The order-app repo has zero `File.Replace` sites for exactly this reason and has pinned it as an invariant (their H-6 §6.10). **Sites:** `OhlcCache.vb:99,137` · `SignalEmitter.vb:569` · `SettingsLoader.vb:346` · `SettingsDiffApplier.vb:352` · `TweakerState.vb:162`. **Not a boundary, no settings keys** |
...
| **C1 F1** — `coverage` verb's `Captured` doesn't mean gap-free: a trailing-edge silence (e.g. trades :00–:05 then nothing until the next hour's :30) is charged to the FOLLOWING hour, so the hour it actually started in reads `Captured` despite ~55 min of silence. Not data loss — the next hour (or later ones, if the app never returns) still flags `Defect` — but a reader reasonably takes `Captured` to mean complete, and it does not. Currently stated in the report's own legend line only (the reviewer's own "cheapest correct action now"), not fixed | Small-medium — bound `hourEnd − lastTradeInHour` as a candidate gap against the SAME trailing-evidence boundary `ResolveBoundaryUtc` already computes (naive version false-positives the final hour of every window otherwise); one new fixture | [`c1-session1-review-2026-08-04.md`](c1-session1-review-2026-08-04.md) §F1, `tools/BacktestRunner/CoverageReport.vb` (`AccumulateHourStats`) |
...
**2026-08-03:** **RULED — every spec/brief for a new implementer MUST carry a model + effort recommendation** (trader-directed; binding text in **CLAUDE.md → Collaboration Rules**, worked example [`trade-store-coverage-report-implementer-brief.md`](trade-store-coverage-report-implementer-brief.md) §0). Not a bare tier: it must name *why that tier*, *where that model will specifically slip*, the *escalation trigger*, and a *session split* when the build is large. Placed in CLAUDE.md rather than its own doc deliberately — a convention nobody reads at the moment they need it is not a convention · **C1 FULLY TICKED (D1–D7) and BUILD-AUTHORIZED** — both parts, A49 family, no ⚠ boundary, implementer brief written · **WEEKDAY SCOPE RULED** (correcting J-C): capture stays 24/7, *evaluation* is weekday-only; retires J-C's weekend clause, confirms D4's 300,000 ms on its existing weekday basis, cancels the REST-backfill task, and **Part B stays unconditional**. Tree check found weekday-only is enforced in code in **exactly one place** (`CeilingAudit`) — `AutoTweaker`, `LivePerformanceTracker`, `AnalysisRunner`, `WhatIfRunner` all unfiltered; logged as a build slot, `AutoTweaker` first because it is the only surface that writes settings.
