...
**Stage-1 profitability levers (2026-07-18 addendum):** the consumer-side **session policy gate** (`session-policy-gate-proposal.md`, P1–P5 await trader) adds per-session tier/context subsets + size multipliers at the live-ladder step — soak-review note: policy refusals log as **`refused: policy(<session>/<reason>)`**, a THIRD disposition token counted separately from `refused:` (other gates) and `rejected:` (API), so the review tallies what the policy declined and the book keeps measuring the counterfactual. Full ladder + tracking: `profitability-risk-levers.md`.
...
- **D6 eval-migration analysis pass (B4b follow-up):** the eval stack deliberately kept its ATR yardstick at v51 — `LivePerformanceTracker.FAV_ATR_MULT` const 2.0, the `AnalysisConstants` 1.2/2.0 mirrors, `FailureRateMatrix`, while `AnalysisRunner` **and `AutoTweakerCore`** read the live 1.75 gate (coordinator record addition 07-06). The follow-up migrates the eval/offline barriers onto the logged `Placed*` columns (F4 established the old barriers weren't pure k×ATR anyway). Analysis pass first, code second; do it before trusting any post-v51 failure-rate reading too precisely. **ANALYSIS PASS DONE 2026-07-08** (`d6-eval-yardstick-divergence-2026-07-08.md`): divergence is **stop-side only** and large — eval adverse barrier = raw swing stop (v51 median 8.9×ATR) vs executed placed stop 1.6×ATR (5.5× wider, unreachable intrabar on 93% of rows), so "failure" ≈ window-expiry and post-v51 rates understate stop-out risk (this is the 0.3% adverse-hit explained). Target side aligned (eval uses placed target). **Remaining = code migration** (AdvBar → PlacedStop at `LivePerformanceTracker.vb:931/934` + the offline `FailureRateMatrix` + const re-sync 2.0/1.2→1.75/1.6, then OHLC re-score) — spec-first; two trader decisions pending (replace-vs-dual-track the swing metric; eval-cache rotation).
...
|---|---|---|
| B4b post-ship (§12) | reach-rate vs fallback; STOP_CLAMPED freq (expected high at fixed size); BELOW_MIN_MOVE +4–6pp NY. **First read 07-13:** STOP_CLAMPED binds 96.4% of v51 directional rows (as designed); BELOW_MIN_MOVE 11–29%/day (07-13: 14.7%) vs v50-baseline 38% — the +4–6pp projection is swamped by day-ATR variance, unalarming; reach-rate needs the OHLC walk (rides D4 of the D6 migration). **Related UI observation closed 07-13:** the perf-strip [B]/[T] modes showing identical numbers is CORRECT — verified end-to-end (mode swap, tooltip, walk, 0/1 serialization, aggregator); on the current book zero failure rows carry a target-hit (13 in the whole cache), i.e. the D6 divergence made visible. The gap reopens once the D6 migration puts the placed 1.6×ATR stop under the eval | `placed-geometry-spec-back.md` §5 |
| LONDON struct-target (F3/DG4) | in-bound structural reach still <45% after ≥3 more LON session-days ⇒ London override or bound tightening. **First evaluation 2026-07-15 (5 LON days post-v51, eval-cache join): NOT TRIPPED — structural 49.3% (n=69) vs fallback 60.8% (n=79); inversion narrowed from 33/61 but persists in direction; n small (CI ±12pp) ⇒ no action, re-read at the W6-1 audit re-run. Side-finding: structural rows adverse-first 76.8% vs fallback 45.6% on the post-D6 tight stop — the sharpest W6-1 LONDON-debt quantification yet** | derivation §4 F3 |
| Retune §5 re-check | AFTER the funding build ships: FLAT 60–70% + 3b engagement 15–25%, **per-resolution — both res-1 AND res-3 in-band** is the success criterion. **Joint v52+v53 watch READ 2026-07-17 → PASS (coordinator ruling), #6 green-lit.** Wire-in: Wed 8.1%/96.6%, Thu 6.7%/94.4% (Thu = maintenance-truncated; under-fire = conservative direction). Funding: **mechanism VALIDATED** — res-3 FLAT 32.8% Thu → 63.6% Fri (day-responsive, the structural-always-active bug gone; Fri in-band); pooled bands out (res-1 FLAT 38%, engagement ~61%) on a hot-crowded week — **re-read across next week; FLAT <50% through a CALM stretch too ⇒ T-re-fit evidence pass (T fit given W)** | funding proposal §7 |
| v48 §4a OFI fire rates | any population outside 0.6×–1.5× of DOM 63.2% for 2 consecutive weekday sessions. **First check 07-13 (4 weekday days): PASS — all (day×session) cells in-band** (NY 57.6–69.9%, LONDON 68–70%, ASIA one day 84.9%/n=73 — hot but inside 94.8%; keep an eye on ASIA). 07-13 NY 65.8% (5th NY day in-band); no new ASIA/LONDON cells (Monday collected NY-only) | `v48-ofi-dominance-rebaseline-spec-back.md` §3–4 (reads the `.bak` + new rows) |
