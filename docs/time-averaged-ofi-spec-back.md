...
4. **`LiveMicrostructureEvaluator` (the #3 TAPE strip) deliberately still uses the snapshot `CalcOFI`, not the average.** The live strip is a *raw, fresher-than-the-run* microstructure readout (proposal trio #3, §11 discipline) — an instantaneous imbalance is the right thing there. The averaging is a property of the *verdict-run* OFI input only. No change to the strip; flagged so it isn't mistaken for an inconsistency. **Amendment (ruled 2026-07-02, audit F3/D3): `ExitGuardEvaluator` also deliberately stays on snapshot `CalcOFI`** — raw-tape reaction *between* runs at the 3s cadence is the guard's purpose; the averaged ratio is ~`avg_window_sec`-lagged by construction, so aligning would blunt it. The twitchiness is bounded: a single OFI-adverse can never fire EXIT alone (D3 dropped the Warn tier; EXIT needs 2+ adverse or a structural break), so snapshot OFI only matters as the marginal second vote — and reacting to a real sweep that coincides with a second adverse signal is correct exit behaviour. Consequence accepted: the EXIT GUARD strip and the HOLD\EXIT row can disagree on `OFI-adverse` for the same market state (transient sweep flips the snapshot, not the average). Trader runs the observational watch — strip EXIT latches the next run's row doesn't corroborate; if they read as alarm fatigue, flip to the align-branch in `audit-fixes-2026-07-02-proposal.md` §4 (contained follow-up).
