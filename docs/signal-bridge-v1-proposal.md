...
- **`instance_id`:** GUID generated once at process start (static readonly on the emitter).
...
7. Every consumed signal logged with disposition (`acted` / `refused: <gate>` / `stale` / `skipped`) — local log in v1, feedback file in v2.
