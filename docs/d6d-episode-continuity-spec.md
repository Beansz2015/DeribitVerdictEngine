...
| ⭐⭐ **Share of ALL logged `AbsorptionAggrUsd` sitting on those reads** | **48.6 %** (442,070 of 909,850 USD) — ⚠ **a 6-day point on a LUMPY sum. The 2026-09-13 8-day re-read: 36.4 %, day-block 95 % 25.8–55.4 %; 10 of 1,159 reads carry 56 % of all pressing. [`absorption-episode-age-read-2026-09-13.md`](absorption-episode-age-read-2026-09-13.md)** |
...
⛔ **So on every read where the episode is older than 10 s — 26.0 % of them, carrying roughly a quarter to a half of the pressing (48.6 % on 6 weekday-days, 36.4 % on 8; a lumpy sum, see the §2.2 table) — `absorbRatio` divides TEN SECONDS of pressing by the WHOLE EPISODE's depletion.** **It systematically UNDERSTATES the ratio on exactly the episodes that matter most.**
...
| # | Decision | My read |
|---|---|---|
| **`D-6d.1`** | ✅✅ **RULED (c) BY THE TRADER, 2026-09-11 (UTC): SIDECAR *AND* ONE CSV COLUMN (`AbsorptionShadowAggrUsd`).** ⛔ **THAT FORCES A HEADER ROTATION, DELIBERATELY — the five riders parked in [`trader-tick-queue.md`](trader-tick-queue.md) §3 travel with it.** **Build spec: [`absorption-d2-stage1-rotation-build-spec.md`](absorption-d2-stage1-rotation-build-spec.md).** ⚠⚠ **THIRD DEFEAT OF A CHEAPER READ IN ONE SESSION, AND IT BELONGS IN THE RECORD `CLAUDE.md` KEEPS: my read was (a), the narrower option; the ruling took the one that RECORDS MORE and PAYS A SCHEMA COST NOW rather than deferring it. That is the documented shape exactly — *"defer a schema fix and it accretes"*, and the 2026-09-01 rotation that went past WITHOUT its riders is this project's own proof.** *(The question and the defeated read are kept below, per the quote-and-label convention.)* ~~Sidecar `absorption_episodes.log`, or five new `analysis_log.csv` columns?~~ | ⛔ **DEFEATED. My read was (a) SIDECAR.** ⛔ **Reserved rather than auto-proceeded, because the CSV is the richer-sounding option and "a rotation is expensive" is the exact tell `CLAUDE.md`'s three-step test names.** **My reason is mechanical, not economic: a CSV column can carry ONE scalar per run, where the loss is a distribution over six close reasons across two sides. The column is not richer — it is narrower. Carrying `InstanceId` + `SignalId` makes the sidecar join losslessly, so nothing is given up.** ⚠ **And a rotation drags in five parked riders under a *"NEVER FORCE ONE"* rule** |
...
- ✅ **The 1.7 s median NO LONGER rests on two weekdays — §2.2 re-measures it at 2.15 s over ~6 weekday-days (840 absorption-active reads).** ⚠ **Still short of `D-1`'s ruled ~10 weekday-days**, so the percentiles will move; ~~**the 26.0 % / 48.6 % split is the load-bearing part and both would have to move a long way to change `D-6d.3`.**~~ ⛔ **CORRECTED 2026-09-13 (UTC): the 48.6 % DID move a long way — to 36.4 % on 8 weekday-days — and `D-6d.3` did NOT change, because its argument is mechanical (Stage 2 without `D-2` makes the ratio fall), not a magnitude. The 26 % read share held at 26.5 %. Neither number was ever load-bearing for `D-6d.3`; saying so overstated both. [`absorption-episode-age-read-2026-09-13.md`](absorption-episode-age-read-2026-09-13.md).**
...
- ⚠ **The pressing share (48.6 % on 6 weekday-days, 36.4 % on 8 — a lumpy sum; 10 of 1,159 reads carry 56 % of it) is computed on `AbsorptionAggrUsd` AS LOGGED — i.e. already truncated by the `D-6d` defect.** If Stage 1 confirms spurious truncation, the true share on long episodes is **higher**, not lower.
