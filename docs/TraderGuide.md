...
- The header shows the fallback multipliers, with the target one **session-resolved** — `1.75x target` on NY, `2.00x` on London, `1.25x` on Asia (v51). They only apply when structure doesn't place the level.
...
- **The placed-target row** renders in the arrow form when structure places it: `Target <fallback> --> <placed>  [PLACED @ <placed> (<LABEL>)]` — and that row carries no inline R:R (read the structural rows below the ATR block for the swing R:R). The label says what the target is: `SWING_HIGH_5M` / `SWING_LOW_5M` (a swing), `NEAREST_HVN_ABOVE` / `NEAREST_HVN_BELOW` (a volume wall), `POC`, or `FALLBACK_ATR` (no structure fit — pure ATR, printed with the true R:R inline). A structural target can sit *farther* than the ATR fallback, not just closer — that's the v51 change.
...
**Example:** `ATR ENTRY LEVELS  (ATR 24.60  size ×1.89 | 1.6x stop / 2.00x target | EXEC 3m)` — a London 3-min session (hence the 2.00x session-resolved fallback target). Current ATR is well below its rolling reference, so `size ×1.89` says you can run nearly double size within your risk rules. If the target row shows `Target   59284.2 --> 59333.4  [PLACED @ 59333.4 (NEAREST_HVN_ABOVE)]`, the engine has placed the target at the nearest HVN wall above instead of the 49-point ATR fallback. If the stop reads `[STOP_CLAMPED]`, your true swing invalidation is wider than the 1.6×ATR the engine is showing — don't mistake the tighter displayed stop for your real risk.
...
**Success metric.** A verdict is counted as SUCCESS if price reached the target barrier before hitting the stop barrier, evaluated on 1m bars T+3 through T+15 (1-min sessions; **3-min Asia/London rows walk T+3 through T+45** — the same windows in bars, scaled post-v41 — so their outcomes show up ~45 min later). Ties in the same bar count as FAILURE (conservative-bias rule). `NO TRADE` and `NO TRADE [WEAK X]` verdicts are not counted — only directional signals enter the denominator.
