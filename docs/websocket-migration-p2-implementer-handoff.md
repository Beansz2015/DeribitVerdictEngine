...
| Trades | buffer **superset-consistency** — REST last-N present in the WS buffer, matched as a **multiset on (timestamp, price, amount, direction)** (the newest few may differ by stream timing) + last-trade price/timestamp | superset + last within a few trades |
...
- **Trades matching — `trade_id` is N/A as a CSV column, optional as a DTO field, and not a speed lever.** The check is an **in-memory** buffer compare; the CSV has no per-trade rows, so there's nothing to add a `trade_id` *column* to. `TradeRecord` carries Price/Amount/Direction/Timestamp/Liquidation (no `TradeId`). The **multiset on (timestamp, price, amount, direction)** is robust against same-ms/same-price sweep fills (which collide on timestamp+price alone) and is sufficient for "do the two streams agree." Performance is a non-issue either way — once per ~10s run on ≤5000 trades, O(n) with a HashSet. **Default: don't add `TradeId`.** If the parity *log* ever shows a spurious mismatch from a multiset collision, adding an optional `TradeRecord.TradeId` **DTO field** is cheap (~3 edits: the field + the parse in `DeribitClient.GetRecentTradesAsync` + `DeribitWsFeed.ApplyTrades`; additive, ignored by indicators, zero scoring/CSV impact) and gives exact 1:1 matching — a robustness nicety, not needed up front.
