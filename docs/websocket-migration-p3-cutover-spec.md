...
## 3. Decision 1 — trades-staleness: connection-health gating (SETTLED)
...
- **The flip is a settings edit, not a POCO default change.** Proposal §8 says "default → ws", but flipping the **POCO default** would silently switch every install/silent-defaults path. Instead: **keep the POCO default `"rest"`** and have the trader set `network.transport: "ws"` in `settings.json` after G1+G2 — a deliberate, dated, hot-reloadable, reversible event. This matches the conservative/reversible philosophy and the tested-milestone push workflow. *(Divergence from §8's literal wording — flagged for trader confirmation; recommend the manual flip.)*
- **Rollback** = set `transport` back to `"rest"` (hot-reload, zero code). The per-run `IsDegraded()` fallback already absorbs transient WS failures; rollback is for sustained distrust.
...
- Confirm **`funding_8h` + the 3-min chart** hold parity under the long soak (P2's 8-run bench was clean; 24h is the real test).
