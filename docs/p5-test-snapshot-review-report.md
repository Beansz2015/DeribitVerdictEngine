...
| **F-07** | HIGH (triage: consider CRITICAL) | 11, 12, 13, 14 | ATR ENTRY LEVELS capped row | Two coupled content-parity gaps: (a) the **raw ATR target is dropped** — legacy shows `Target 50160.0 --> 50080.0`, card TARGET cell shows the capped value only; (b) the **R:R cell keeps raw-ATR content** — ratio `1:1.7` plus subline `(risk 96.0 / rwd 160.0)` (read via 3× zoom) — next to the capped TARGET. Legacy deliberately omits R:R on capped rows. A trader can read R:R 1:1.7 / rwd 160 against a target whose actual reward is 80 points. KNOWN-0 currently hides the subline, but the 1:1.7 ratio is visible. | `11__zoom_rr_capped.png`, `13__c2z_atr_money.png` |
