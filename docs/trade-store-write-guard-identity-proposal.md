...
| **D-7** | ⚠ Measure §5b before shipping? | yes / no | **YES, and it is time-boxed.** The Deribit maintenance window will leave a **complete** repair-written band between two ~50 % streaming bands — a third independent confirmation, with boundaries known in advance. **Shipping the fix first destroys it**, because the repaired band becomes indistinguishable from correctly-streamed tape. Take the measurement at the next copy-back, then ship |
