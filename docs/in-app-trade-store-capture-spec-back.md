...
**The row PARSE.** `HistoricalStore.LoadTradeRange` now delegates its per-file read to `TradeStoreWriter.ReadTradeFile`. Putting the parse on the seam makes the A48a round-trip exercise shipped code, and satisfies §2's stated goal better than the original split did — "the format lives in exactly one place, so the writer and the reader cannot drift" is more true when the reader is also there.

> **CORRECTION 2026-07-31.** This paragraph originally justified the move by claiming "the fixture project deliberately does not link `HistoricalStore` (it owns the `HttpClient` the §2 split exists to keep out)." **That is false.** `verify/ordercheck/OrderCheck.vbproj:146` has linked `HistoricalStore.vb` since `af746cb` (the A43 family) — before this build existed. The move is still right, on the reason given above; the claim I used to support it was not checked and was wrong. The §2 split's real value is keeping an `HttpClient` off the **app's feed path**, which stands.
