...
| Version | Key Changes |
|---|---|
| **2026-05-17** | [display-polish-pass] `settings.json` bumped v29 → v30. Spec: `docs/display-polish-pass-proposal.md`. Eight display-side fixes bundled in one commit, all zero-behaviour-impact. **F1** `AnalysisOutputDump.Append` gains optional `perfStripLine` param; `MainForm_Render_Sections` builds it from `lblPerf*.Text` + `_metricMode` (`[B]`/`[T]`) and passes through so each dump block carries the perf strip the user saw. **F2** R:R formatter (`MainForm_Render_Header.FormatRR`): `0` → `1:0.0`, `0 < ratio < 0.1` → `1:< 0.1`, else 1dp. Replaces inline format calls in both structural rows. **F4** Near-zero funding rates clamped to `0.0` (threshold `1e-8`) at both display sites — FUNDING section row in `_Sections` and `Funding (info)` row in `ScoringEngine_Calculate_Scoring`. **F6** Sub-noise CAPPED label suppression: when `|raw − adjusted| < max(0.5, ATR × 0.02)`, the amber-bold `[CAPPED @ ...]` annotation is hidden and the adjusted target rendered as a normal value. `TargetCapReason*` still populated on `VerdictResult` for CSV. **F8** Kelly `Lean:`/`Contracts:` rows pluralise on count (`1 contract` / `N contracts`). **F9** `OI Delta` SIGNAL BREAKDOWN row precision raised 2dp → 3dp to match OPEN INTEREST section. **F11** `CalcVerdictContext` returns `ALIGNED` (new value) instead of `CONFIRMED` when verdict starts with `"NO TRADE"`; CSV `VerdictContext` column may now contain `ALIGNED` on NO TRADE rows post-v30 (existing rows unchanged). **F12** Per-side missing-target/missing-stop wording: long row's missing-stop says `"stop unset: no swing low below entry within lookback"` / missing-target says `"target unset: no swing high above entry within lookback"`; short row mirrors. Removes the mental inversion of the old shared note. No new config keys; thresholds hardcoded. |
| **2026-05-17** | [auto-tweaker-fixed-window] `settings.json` bumped v28 → v29. Spec: `docs/auto-tweaker-fixed-window-proposal.md`. Auto-tweaker switches from sliding to fixed (non-overlapping) windows so consecutive BELOW_THRESHOLD rounds are statistically independent. **`TweakerConfig`** gains `WindowMode` (`"fixed"` default, `"sliding"` retained as deprecated) and `MinTierEligibleRows` becomes `Integer?` (nullable) with new `EffectiveMinTier(windowSize)` / `ComputeDefaultMinTier(windowSize)` helpers — null auto-scales as `max(15, ceil(WindowSize × 0.5))`. `CooldownRows` retained but logged once and ignored when `window_mode=fixed`. **`TweakerState`** gains `LastEvaluatedRowIndex` (default -1 = uninitialised); on first v29 run it is seeded to `currentRowCount` so historical sliding-era CSV data stays in the file but is not re-evaluated under fixed semantics. `RoundHistoryCap` raised 50 → 1000. **`AutoTweakerCore.RunAsync`** branches on `WindowMode`: fixed-mode builds the window as `allRows[LastEvaluatedRowIndex .. +WindowSize-1]`, runs the session-boundary scan within that disjoint slice, then the tier-eligible check. Tier-eligible failure in fixed mode writes a new `SKIPPED_INSUFFICIENT_TIER` `RoundSummary`, advances `LastEvaluatedRowIndex += WindowSize`, but does **not** tick `CurrentBelowThresholdStreak`. Session-boundary failure in fixed mode writes `SKIPPED_SESSION_BOUNDARY` with identical streak/advancement semantics. Every completed terminal branch (BELOW_THRESHOLD / DRY_RUN_WRITTEN / APPLIED / PROPOSED / REVERT) also advances `LastEvaluatedRowIndex += WindowSize`. Sliding branch preserved verbatim under the `Else` arm with the only behavioural change being that its tier-eligible check also consults `EffectiveMinTier`. **`UI/TweakSettingsForm.vb`** gains a "Min tier-eligible rows" textbox row between Cooldown rows and Snapshot history, with a tooltip explaining the auto-scale formula. The textbox tracks the WindowSize-derived default until the user types into it (`_minTierIsAuto` flag); on Save the value is persisted as `null` when blank or still tracking, otherwise as the explicit integer. Save validation: reject `< 5` ("Min ≥ 5 required for any statistical meaning."), reject `> WindowSize` ("Cannot exceed Window Size — gate would be unreachable."), warning MessageBox at `> WindowSize × 0.7` (proceeds on OK). `UpdateStatusLabel` rewrites the live status flow for fixed mode: `Awaiting first-run seed (rows so far: N)` while uninitialised, `Next round: K/N rows accumulating (rows X..Y)` while the disjoint window fills, and a `Window full but tier-eligible {n}/{min} — round will SKIP` warning when the prospective round would be skipped. |
| **2026-05-17** | [target-hit-metric-toggle] `settings.json` bumped v27 → v28. Spec: `docs/target-hit-metric-proposal.md`. Decouples direction-prediction quality from stop-placement quality on the live performance strip. **Eval cache schema bumped v1 → v2** with new `TargetEverHit` column (`1` / `0` / empty); `LivePerformanceTracker.IsV1Schema` detects v1 by absence of `"TargetEverHit"` in the header; new private `MigrateV1ToV2` (called from new Step 2.5 in `InitialiseAsync`) walks every non-EXCLUDED, non-PENDING row against `_ohlcLookup` using the new `FailureRateMatrix.TargetHitWalk(bars, favBar, isLong)` helper and rewrites the file with the v2 schema comment + header. SUCCESS rows always backfill to `TargetEverHit=True` by construction. Rows with no OHLC coverage stay `Nothing`. Idempotent on subsequent restarts. **`EvalCacheEntry`** gains `TargetEverHit As Boolean?`. **`WindowAggregate`** gains `TargetHitCount` and dual `BarrierRatePct` / `TargetRatePct` read-only properties (replacing single `RatePct`); same denominator `(SuccessCount + FailureCount)` for both rates. `BuildAggregate` increments `TargetHitCount` for every row whose `TargetEverHit.HasValue AndAlso .Value` regardless of barrier outcome. **`EvaluateEntry`** return type changed to value tuple `(outcome As String, targetHit As Boolean?)`; uses `TargetHitWalk` alongside `WalkBars` in a single pass. `ResolvePendingRows` and the analysis-log-backfill site unpack the tuple. `ParseEvalLine` accepts 6- or 7-column rows; `FormatEvalEntry` writes the 7th column as `"1"` / `"0"` / `""`. **`PerformanceDisplaySettings`** gains `MetricMode` (`"barrier"` default). **`MainForm_Layout`** gains `lblPerfMode` (small `[B]` / `[T]` indicator just before `lblPerfWeek` on the perf strip; `C_DIM` when ephemeral mode matches the settings.json default, `C_WARN` when overridden), `_metricMode` field (in-memory ephemeral state, finalised from settings.json after `SettingsLoader.Initialise`), `_perfContextMenu` (2-item right-click menu: `"Use barrier metric"` / `"Use target metric"`). Left-click on any of the six rate labels toggles `_metricMode` ephemerally with a status-bar prompt; right-click opens the context menu, which calls `PersistMetricMode` → `SettingsLoader.Save(cfg, "performance_display.metric_mode → <mode>")`. **`UpdatePerformanceLabels`** picks the active rate (`BarrierRatePct` vs `TargetRatePct`) and appends a second tooltip line `"{Other}: {pct}% ({hits}/{n})"` so both metrics are visible without toggling. `ResizeControls` adds `lblPerfMode` as the first element of the cascaded perf-strip layout. |
