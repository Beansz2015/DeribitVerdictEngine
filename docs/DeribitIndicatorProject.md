...
|---|---|---|
| **Gap repair by `trade_seq` range — `HistoricalStore` seq fetcher + `Core/RepairStatusLog.vb` + `collector.ps1` + fixtures A79a–A79g** (tape-path engine change, settings-untouched) | 2026-09-14 | Per [`gap-repair-same-ms-page-skip-spec.md`](gap-repair-same-ms-page-skip-spec.md), trader-ruled `GR-1` (d), `GR-2` (b), `GR-4` (b), `GR-5` (b). The time pager's `newestMs + 1` cursor skipped any trade sharing a full page's last millisecond, and the time hole windows `[L.ts + 1, R.ts − 1]` could never fetch it back: 16 permanent holes (70 trades) in the 2026-08-17 start-up repair. Repair now fetches each hole's missing `trade_seq` range and each tail from the last stored seq + 1 via `get_last_trades_by_instrument`, with one `count=1` time anchor for an empty file; the time pager is deleted, offline path included. Not-served seqs (the venue's retention edge) are counted, never assumed away. New `repair_status.log` (4 fields, one PASS line per pass) fetched by `collector.ps1`. `A56a`–`A56f` remapped to seq ranges; `A79g` pins that a seq tail beside an unflushed streaming writer still double-writes (readers dedupe; fix belongs to the duplicate-rows task). Harness 382 to 389. No scoring impact, no rendered surface, not an `analysis_log.csv` dataset boundary; a store-completeness edge at the deploy's `InstanceId`. The S2 deploy waits for this build. Settings v68 untouched |
