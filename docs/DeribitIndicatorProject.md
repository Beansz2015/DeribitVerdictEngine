...
- **CalcKellySizing():** called inline from `BuildPlaintextSnapshot` (`UI/MainForm_PlaintextSnapshot.vb`) before the card binds. Display-only, zero scoring impact. **[v69]** p and b come from `LivePerformanceTracker.ComputeKellyBook` (the live eval cache, pooled per session), split into 3 terciles of the book's own placed net payoff (`docs/kelly-one-class-placed-payoff-spec.md`, K-1 = (g)) — no longer a confidence-tier map. Original line: [`history-archive.md` §J, `trim-2026-09-14b-01`](history-archive.md#trim-2026-09-14b-01)
...
- **Kelly Sizing block** rendered after ATR levels. Half-Kelly, 5% hard cap, $1,000 account, $10 contract face. **[v69, `kelly-one-class-placed-payoff-spec.md`]** p and b are BOOK mode — measured on the live eval cache, pooled per session and split into 3 geometry terciles (K-1 (g)); the EST confidence-tier map is retired (`kelly.est_prob_floor`/`est_prob_scale` removed). Shown whenever a Kelly side exists (verdict side, or the lean on `NO TRADE [WEAK LONG]`/`[WEAK SHORT]`): a below-floor state when the session book is under `kelly.min_book_rows` (400), a `[NO EDGE]` state at f* ≤ 0, full sizing at f* > 0. Hidden only on plain `NO TRADE` / `NO TRADE [TIE]` (no side). v30 plural fix: `1 contract` / `N contracts`. Original line: [`history-archive.md` §J, `trim-2026-09-14b-02`](history-archive.md#trim-2026-09-14b-02)
...
- `AnalysisLogger.LogRun(r, verdict)` → `analysis_log.csv` in exe directory. The header carries 124 columns since the S2 rotation of 2026-09-24 (counted from `AnalysisLogger.Header` on 2026-09-25; it read 116 before). A header change rotates the book to a `.bak`, and every rotation rider is listed in [`csv-rotation-riders.md`](csv-rotation-riders.md). Original line: [`history-archive.md` §J, `trim-2026-09-14b-03`](history-archive.md#trim-2026-09-14b-03)
...
|---|---|---|
| **v52 aggressor-velocity wire-in post-ship watch (S5.2)** | NY-by-1-min watch: burst fire rate 8-12 %, same-side share (TFI on the burst side) at least 85 %, TFI-modifier engagement about 5-10 % of NY directional votes. Trigger: out of band on 2 consecutive weekday sessions, then re-run the derivation in `aggressor-velocity-s52-derivation-2026-07-13.md` §5.2 and §7. The res-3 display-only clause is spent (LONDON armed v60, ASIA v65). ASIA watch second read 2026-09-14: MISS at 7.75 %, re-derivation read owed — [`d3-asia-burst-watch-read-2026-09-14.md`](d3-asia-burst-watch-read-2026-09-14.md). ⚠ **2026-09-26:** the re-derivation read found the fire rate volatility-conditional in all three sessions; NY reads 4.60 % since 2026-08-20, out of this band. Trader-ruled `AVR-1` (b): the reference becomes ATR-conditional for every session; `AVR-2` (a): no threshold change until an outcome read — [`aggr-vel-burst-rederivation-read-2026-09-26.md`](aggr-vel-burst-rederivation-read-2026-09-26.md). Full cell: [`history-archive.md` §I, `trim-2026-09-14-03`](history-archive.md#trim-2026-09-14-03) | Medium (watch) |
...
|---|---|---|
| **`RR-1` gap-repair sort key — `ResolveRepairWindowsCore` now sorts primarily by `TradeSeq`** (repair-path change, no scoring impact, no CSV boundary, settings-untouched) | 2026-09-25 | [`gap-repair-rr1-seq-order-spec.md`](gap-repair-rr1-seq-order-spec.md), `SO-1`=(a). The old `(Timestamp, TradeSeq)` sort let a same-burst timestamp inversion (higher seq, earlier ms) split a contiguous seq run into phantom holes — measured 5 phantom re-detections / 19 duplicate rows over 2.5 days ([`gap-repair-repeat-fills-read-2026-09-25.md`](gap-repair-repeat-fills-read-2026-09-25.md), `RR-1`). Seq-carrying pairs now sort by `TradeSeq`; a legacy row still sorts by Timestamp against its neighbours (`SO-2`/`ST-1` prove this is a valid total order). Fixtures `A91a`-`A91c` (mutation-proved); `A56c`/`A56d` UNCHANGED. Harness 483→486, Release 0/0, `verify-gate.ps1` PASSED. **DEPLOYED 2026-09-25 13:31 UTC** (instance `a19acc4d…`); the live proof (no range repaired twice) is owed at the next copy-back. Settings v69 untouched |
| **Kelly as one class, p/b from the book's placed levels net of fees** (RENDERED-VALUE CHANGE; NOT a CSV boundary; no scoring impact; settings v68→v69 — `kelly.min_book_rows`=400 added, `kelly.est_prob_floor`/`est_prob_scale` removed) | 2026-09-25 | [`kelly-one-class-placed-payoff-spec.md`](kelly-one-class-placed-payoff-spec.md), K-1=(g). `LivePerformanceTracker.ComputeKellyBook` pools the live eval cache per session, splits into 3 terciles of the book's own placed net payoff; a row's p/b are its bucket's measured values, falling back to the session pool below the floor. Gate moved `KellyPWin>0` → `KellyHasSide` (F-1). Three states: below-floor, `[NO EDGE]` (f*≤0), full sizing. Real 2026-09-25 08:53 UTC cache: f*<0 in every session/bucket (no escalation). `KELLY_CARD_H` 220→280px, screen-verified. Fixtures `A10` rewritten + `A90a`-`A90k`, harness 468→483. Packets: `kelly-one-class-placed-payoff-batch-summary.md` / `-spec-back.md`. This row pushed v64 (2026-07-31) out of the kept-5 window; v64 moved to `history-archive.md` §E in `0ba8d9b` |
| **Absorption `D-2` + `D-6d` Stage 1 + the `analysis_log.csv` rotation** (LIVE-STRIP CHANGE AND A DATASET BOUNDARY for `AbsorptionAggrUsd` / `AbsorptionRatio`; header 116 → 124 columns; no scoring impact while `scoring_enabled` is false; settings-untouched) | 2026-09-24 | [`absorption-d2-stage1-rotation-build-spec.md`](absorption-d2-stage1-rotation-build-spec.md). S1: pressing is episode-cumulative (`D-2`; queue, `PressQueueCap` and prune removed, `window_sec` unused); shadow press, seven close reasons and `absorption_episodes.log`. S2: eight columns appended after `AbsorptionSizeMin` — `AbsorptionShadowAggrUsd`, `TriggerMode`, `WsHealth`, `SettingsVersion`, `SettingsLoadError`, `RecentTradeCount`, `VPFRSignal`, `VPFRPoc`; the rotated book is named from its own header; both ops scripts pool every rotated book. Riders `RIDER-1`–`RIDER-7`, `RIDER-9` consumed. The live strip ratio moves; snapshot and cards do not. Fixtures `A88a`–`A88f`, `A89a`–`A89h`, `A60e` extended. Harness 453 to 468. Packets: [`absorption-d2-s1-spec-back.md`](absorption-d2-s1-spec-back.md), [`absorption-d2-s2-spec-back.md`](absorption-d2-s2-spec-back.md). NOT DEPLOYED. Settings v68 untouched |
| **`D-8` report lean column + `D-9` Step 3b effect on both surfaces** (rendered values only, NOT a dataset boundary, settings-untouched) | 2026-09-24 | `D-9`: new `VerdictResult.FundingStep3bLongPoints` / `ShortPoints` carry Step 3b's actual effect (after its clamps); scores unchanged. New `Core/FundingStep3bDisplay.vb` feeds both surfaces: the snapshot `Momentum:` line now prints `Effect: -1[L]` in place of `Soften`/`Amplify`; the card's `Config:` row became `Step 3b:`; three card colour sites follow the effect's sign. `D-8`: the offline report's §6 table keeps directional trades, lean NO TRADE rows (walked as if taken) and `[TIE]` counts in three labelled columns. Fixtures `A86a`–`A86c`, `A87a`–`A87c`. Harness 435 to 453. Spec: [`engine-fix-build-spec-2026-09-21.md`](engine-fix-build-spec-2026-09-21.md) §5; spec-back [`engine-fix-session-c-spec-back.md`](engine-fix-session-c-spec-back.md). Settings v68 untouched |
| **`D-1` POC-tier gate fix + `D-2` manual geometry** (LIVE SCORING-OUTCOME CHANGE AND A DATASET BOUNDARY, settings-untouched) | 2026-09-24 | The POC-tier gate now opens the long tier on `NEAR_HVN_SUPPORT` and the short tier on `NEAR_HVN_RESIST`, the labels `CalcVPFRLite` emits when the POC sits on that target side. Both copies changed: `pocGated` in `SignalEmitter` and the legacy twin `hvnAbove`/`hvnBelow`. The `IN_LVN_*` halves did not move. Measured on the pooled book: 1,288 of 38,665 verified rows get a different placed target; 143 of 8,508 population rows (1.68 %) become NO TRADE through Step 5c. Rows either side of the deploy are not comparable for `TargetCapReason`, `Placed*` and those verdicts. Fixtures `A80b` (now always-on), `A80c`, `A83a`, `A83b`; `A26b` and one `A82b` site made producer-consistent. Harness 425 to 435. Spec: [`engine-fix-build-spec-2026-09-21.md`](engine-fix-build-spec-2026-09-21.md) §3; spec-back [`engine-fix-session-a-spec-back.md`](engine-fix-session-a-spec-back.md). Settings v68 untouched |
| **Gap repair by `trade_seq` range — `HistoricalStore` seq fetcher + `Core/RepairStatusLog.vb` + `collector.ps1` + fixtures A79a–A79p** (tape-path engine change, settings-untouched) | 2026-09-14 to 2026-09-15 | Per [`gap-repair-same-ms-page-skip-spec.md`](gap-repair-same-ms-page-skip-spec.md) (`GR-1` (d), `GR-4` (b)). The `+ 1` time pager lost same-ms trades for good (16 holes, 70 trades); repair now fetches missing `trade_seq` ranges and counts not-served seqs. `F-1` ([`gap-repair-cross-month-gap-spec.md`](gap-repair-cross-month-gap-spec.md)): a gap across 00:00 UTC on the 1st is repaired. `DUP-1`/`DUP-2` ([`gap-repair-scan-failure-spec.md`](gap-repair-scan-failure-spec.md)): a failed store scan is `SCAN_FAILED`, never an empty store (it rewrote 298,932 rows in September); repair scans share the file and stop at the last complete row. New `repair_status.log`, fetched by `collector.ps1`. Harness 382 to 398. No scoring impact, no rendered surface. The S2 deploy waits. Settings v68 untouched |
...
Older entries are **not** deleted — **v64 down to v27** live verbatim in [`history-archive.md`](history-archive.md) §E, and v26 back to v0.33 below them.
