...
| **v29 first auto-tweaker fire (NEXT — sequencing set v34)** | The fixed-mode tweaker has never fired live. Run the first fire as a **dry-run** (`dry_run_enabled: true`) once ≥1 full clean **v34-scored** window exists — primarily to validate snapshot/apply/revert/streak mechanics, trader reviewing the proposed diff. **Prefer a weekday window** (the primary NY regime) so proposals aren't biased by weekend microstructure; **hold any `session_volume` change it proposes** until the weekday-ASIA re-verify (below). Not gated on weekday-ASIA collection — the two are orthogonal. | Medium |
