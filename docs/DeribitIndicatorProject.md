...
Current version: **v35**. Top-level blocks:
...
| **Weekday-ASIA `session_volume` re-verify (v34 confound)** | v34 set ASIA `high/mid_mult` to 1.10/1.05 on a sample that was **100% Saturday** (weekend); the NY baseline was 100% weekday, so the session comparison is weekday/weekend-confounded — weekday Asia (UTC 0–7 Mon–Fri) is absent. The over-trade *direction* is robust (0.80/0.85 lowered the bar on any day) but the *magnitude* is weekend-set. **Once ≥50 weekday-Asia rows exist, re-verify by hand** (NOT via the auto-tweaker — it tunes the tier failure-rate matrix and has no day-of-week concept): recompute ASIA trade-rate / RANGE_BOUND % / VolumeRatio tail on weekday-only Asia rows; if weekday Asia is materially calmer than the Saturday sample, dial 1.10/1.05 back toward neutral (1.00/1.00). **Bundles the offline Deribit volume re-fetch** (re-anchors OBV's `|obvChange|` median on multi-session data + runs the ASIA/LONDON clamp-binding check + computes the ASIA fire-rate — closes the v34 OBV/method follow-ups). Runs **after** the v35 de-confound pair, before the first fire. Spec: `clean-data-rebaseline-v34-proposal.md` §3.1 + spec-back §7. | Medium |
