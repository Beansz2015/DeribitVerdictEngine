...
|---|---|---|
| **Gap repair by `trade_seq` range — `HistoricalStore` seq fetcher + `Core/RepairStatusLog.vb` + `collector.ps1` + fixtures A79a–A79k** (tape-path engine change, settings-untouched) | 2026-09-14 | Per [`gap-repair-same-ms-page-skip-spec.md`](gap-repair-same-ms-page-skip-spec.md) (`GR-1` (d), `GR-2` (b), `GR-4` (b), `GR-5` (b)). The time pager's `newestMs + 1` cursor and `[L.ts + 1, R.ts − 1]` hole windows lost same-ms trades for good (16 holes, 70 trades, 2026-08-17). Repair now fetches each hole's missing `trade_seq` range and each tail from the last stored seq + 1; not-served seqs are counted; the time pager is gone. `F-1` ([`gap-repair-cross-month-gap-spec.md`](gap-repair-cross-month-gap-spec.md)): a month file with no row below its segment start seeds its bracket from the previous month's newest row, so a gap across 00:00 UTC on the 1st is repaired. New `repair_status.log`, fetched by `collector.ps1`. `A79g` pins a streaming double-write left to the duplicate-rows task. Harness 382 to 393. No scoring impact, no rendered surface, not a dataset boundary. The S2 deploy waits. Settings v68 untouched |
