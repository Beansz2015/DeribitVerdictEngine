...
|---|---|---|
| **v43** | 2026-06-25 | **Realtime exit guard — P4 #1** (display/alert-only; **zero scoring/CSV/dataset impact**; settings v42→v43). Spec: `realtime-exit-guard-proposal.md` (APPROVED 2026-06-24). ONE new top-level block `exit_guard {enabled:true, interval_sec:3, debounce_evals:2, sound_enabled:true}`. While a position is declared, a WinForms timer re-runs the **fast microstructure exit checks** (the two EXIT-producing `CalcHoldStatus` layers — 2+-adverse fast exit + structural-break exit — plus the single-adverse soft warning) against the **live WS `MarketState`** every `interval_sec`, surfacing an **EXIT GUARD** strip in the LOG cascade + an optional alarm — exit cues at tick freshness instead of the 10–60s poll. It **never** calls `Calculate()`, writes the CSV, or changes the verdict. **Code:** shared `ScoringEngine.ComputeFastExitPrimitives` extracted from `CalcHoldStatus` (Layers 1/1.5/3 refactored to consume it — **output byte-identical**, the single source of truth so the overlay can't drift on "what counts as adverse"); new host-agnostic `ExitGuardEvaluator.vb` (recomputes MicroCVD/TFI/OFI/CVD from `MarketState` via the existing pure indicator fns, **same cfg params + same 500-trade window** as the full run — fresher data, identical method); WinForms host (`MainForm_ExitGuard.vb`) = a `System.Windows.Forms.Timer` started/stopped with the auto-run lifecycle, the §4.1 gate (enabled + `posState≠None` + WS connected/not-cooling/fresh — **paused on stale/down feed, never fires on stale data**), the strip + `System.Media` on the EXIT **latch** transition (debounce `debounce_evals`, auto-clear + re-arm). WS-mode feature by nature (reads `MarketState`); at `transport=rest` it shows **"WS only"**/hidden. **OFF the auto-tweaker surface:** `"exit_guard."` added to `SettingsDiffApplier` `RejectedPathPrefixes` + PromptBuilder **HARD CONSTRAINT 13** (trader-risk/display preference, same class as `kelly.*`). **Live status-bar element** like `BuildWsStatusSegment` — NOT an RTF/snapshot/card surface, so **no card-binding obligation** (engine display-string parity rule, spec §6). Acceptance: solution(Release) + AutoTweaker + OrderCheck build **0/0**; `CalcHoldStatus` **byte-identical** (new harness **A17g** asserts the exact Layer-1 string through `Calculate()`/InLong); evaluator fixtures **A17a–A17f** (2-adverse/structural → Exit, single → Warn-count, 0 → Clear, mirror short, empty buffer → Clear); A1–A16e unregressed; `transport=rest` byte-identical (guard host-gated, verdict path untouched). No CSV/eval-cache change (CSV v0.7, eval cache v4). **Two host-layer decisions to flag for the trader (easily flipped on test):** (1) the **Warn** state fires on *any* single adverse signal (matching the §4.6 mockup's TFI example) rather than the §4.2 micro-only parenthetical; (2) the strip detail (adverse-signal list / break level) rides a **tooltip** so the inline LOG-cascade line stays glanceable in the half-width LOG box. Local commits only — trader tests + pushes. |
