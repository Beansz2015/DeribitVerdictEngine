...
|---|---|---|
| **C1 Session 2 rider · dead-capture-path escalation fix (settings-untouched)** | 2026-08-05 | **Orchestrator review finding (`c1-session2-review-2026-08-05.md`, medium, not blocking): `ClassifyTapeStoreTier`'s `UNKNOWN` state was unbounded in time.** A48e already pins that an unwritable store (blocked directory, locked file, full disk) NEVER THROWS — `AppendRows` logs and returns 0 forever — so a permanently dead capture path and a genuine cold start were indistinguishable: both read `secondsSinceFlush = Nothing` forever, and the strip showed `TAPE STORE: no flush yet` in neutral colour indefinitely. That is precisely the failure Part B exists to surface, rendered as benign, on the only box that captures — and the review noted it blocks safely retiring the daily RDP glance, which is most of Part B's value. **Fix:** `ClassifyTapeStoreTier` gains a second parameter, `secondsSinceStart` (time since the status element's own tracking began — stamped once in `MainForm_TapeStoreStatus.StartTapeStoreStatus`, restarting naturally every process start). While no flush has EVER landed, the tier now escalates `UNKNOWN → AMBER → RED` on the SAME 3×/10× thresholds a stale-but-once-working flush would trip, instead of staying `UNKNOWN` forever; a genuine cold start still reads neutral inside the grace window. The composed strip text also now shows the elapsed seconds in the "no flush yet" case, so a stuck read grows a visible number instead of a static, unchanging string. **New fixture A49n** pins all five arms: genuine cold start stays `UNKNOWN`; past the amber horizon with no flush ever ⇒ `AMBER`; past the red horizon ⇒ `RED`; a stale-but-once-working flush is unaffected; a healthy recent flush stays `NORMAL` regardless of process age. **No settings keys, no version bump** — settings stays v65. Acceptance: solution + OrderCheck build **0/0** Release; harness **ALL PASS** (A1–A49m unregressed + new A49n); verify-gate `prepush` GATE PASSED. The review's other note (§2a, REST-transport boxes show no strip at all since a different writer instance carries capture there) was explicitly **noted, not asked for** — not addressed here. The §15-regrowth the review also flagged is the reviewing seat's own follow-up, not this commit's. |
| **C1 Session 2 · live TAPE STORE status strip (settings-untouched)** | 2026-08-05 | **Part B of the trade-store coverage report — a live app-side capture-health element. NO settings keys added (derives its cadence + amber/red thresholds from the existing `trade_store.flush_seconds`), so NO version bump — settings stays v65.** Spec: `trade-store-coverage-report-proposal.md` §4; orchestrator review of Session 1: `c1-session1-review-2026-08-04.md` (✅ ACCEPTED, two non-blocking findings, both addressed — see below). Session-1/Session-2 split per `trade-store-coverage-report-implementer-brief.md` §0. **Mechanism:** `Core/TradeStoreWriter.vb` gains `LastFlushUtc`/`TotalRowsWritten` instance state (wall-clock facts about the writer's own life — a flush proves the WHOLE chain to disk, a buffered trade only proves the stream) + the pure, host-agnostic `ClassifyTapeStoreTier(secondsSinceFlush, flushSeconds)` (amber past 3× flush_seconds, red past 10×; deliberately takes no date input at all — the absence IS the "stays unconditional on weekends" guarantee). `DeribitWsFeed.GetTradeStoreStatus()` is the read; new `UI/MainForm_TapeStoreStatus.vb` is the label (own `System.Windows.Forms.Timer`, ticking every `flush_seconds`, independent of auto-run/exit-guard/live-strip — the established per-feature-timer pattern). Renders `TAPE STORE: 12s · 47.3k rows` in its own AutoSize row in the SETTINGS & TOOLS card (`outer` TLP grew from 4 rows to 5, between MIN NET MOVE % and TOOLS), collapsing to 0 height when `trade_store.enabled` is false — **zero visual change on a non-capturing box** (the local box, under D1's AWS-only ruling) — `SyncSettingsCardHeight` extended to the same shape it already uses for the EXIT GUARD strip. **Display-string parity: NO OBLIGATION** — a live status element, the v62 MIN NET MOVE % / EXIT GUARD strip precedent (no snapshot line, no card binding). **Session 1 review — both findings addressed:** F1 (`captured` doesn't mean gap-free — a trailing-edge silence is charged to the FOLLOWING hour) gets the reviewer's own "cheapest correct action now": a legend line in the coverage report's own console/markdown output, stated rather than fixed blind (the bounded fix needs care against `ResolveBoundaryUtc` and risks a worse mis-attribution under time pressure — left as a named follow-up). F2 (a capture-state transition mid-hour is scoped by the PREVIOUS marker) is flagging-only per the review (a spec question, not an implementation one) — recorded, not changed. **Fixture A49m** (the one deferred from Session 1): a Saturday hour classifies `out-of-scope-weekend` via Part A's `ClassifyHour`, paired with Part B's `ClassifyTapeStoreTier` reporting `RED` on the identical silence duration — proving Part B has nothing in it that could suppress a weekend reading. **Not launched live** — the same "v57 stomp" precedent the fee-aware min-move build (v62) recorded: starting `MainForm` appends collector rows under a fresh `InstanceId` into the real dataset, so the row/RowStyle/control-index arithmetic was hand-verified instead (5 RowStyles ↔ 5 `Controls.Add` calls, indices 0–4 all accounted for) and the visual render is the trader's test gate, same as the MIN NET MOVE % row before it. Acceptance: solution + AutoTweaker + WhatIfRunner + CeilingAudit + BacktestRunner + OrderCheck build **0/0** Release; A1–A49l unregressed + A49m; verify-gate `prepush` GATE PASSED (expect the same non-blocking version-bump WARN as Session 1, for the same reason). Local commit — trader tests. |
| **C1 Session 1 · trade-store coverage report `coverage` verb (settings-untouched)** | 2026-08-04 | **New `BacktestRunner coverage` verb + D7's per-process capture-scope marker. Tools-only + one silent app-side write; NO settings keys added (reads existing `trade_store.enabled`/`store_dir`), so NO version bump — settings stays v65; NOT a dataset boundary.** Spec: `trade-store-coverage-report-proposal.md` (BUILD-AUTHORIZED 2026-08-03, all seven D's ticked); implementer brief: `trade-store-coverage-report-implementer-brief.md`; rulings: `j-b-scoping-ruling-2026-08-02.md` + `weekday-scope-ruling-2026-08-03.md`. **Session 1 of 2** — Part B (the live `TAPE STORE` status element) + fixture A49m are Session 2. **Mechanism:** six-class-per-weekday-hour classification (`captured` / `defect` / `expected-missing` / `not-capturing` / `unknown-scope` / `out-of-scope-weekend`) over S0 (optional `--verify-venue` diff, pure + injectable, no live HTTP in fixtures) / S1 (uptime cross-reference: `analysis_log.csv` primary + `ws_health.log` supplement, merged into per-instance up-intervals) / S2 (empty hours) / S3 (longest-gap-vs-threshold, default 300,000 ms, CONFIRMED not provisional per the weekday ruling) / S4 (candle + funding completeness, reusing the shipped `StoreFiles`/`HistoricalStore` grid helpers, no second parse). New host-agnostic `Core/CaptureMarkerLog.vb` (D7) — one line per process at start (`utc \| enabled \| store_dir \| instance_id`), reading the MERGED `SettingsLoader.Current` (never the tracked base — a base read would misreport every local overlay-off hour as a defect), following the `WsHealthLog`/`AlertsSidecar` never-throws contract; wired into `MainForm_Layout.vb` beside the existing `WsHealthLog.LogStart` call. New tools-only `tools/BacktestRunner/CoverageReport.vb`. **Two decisions surfaced during the build, both recorded in the file header for the reviewing seat:** (1) `expected-missing` fires ONLY for hours strictly before the very first evidenced process life (clean, unambiguous); every other "no uptime evidence" shape (a cross-GUID restart gap, or the open trailing window) is ambiguous and defaults to `defect` per J-B — UNLESS the store itself shows clean data for that hour, which always wins as `captured` regardless of how ambiguous the uptime read is. (2) S1 is skipped entirely only when BOTH `ws_health.log` is absent AND `analysis_log.csv` carries zero rows in range — not merely when `ws_health.log` alone is missing, since the proposal's own revision makes the CSV the PRIMARY record precisely so its absence must not blind the report to a missing supplement file. **A real implementation bug found and fixed via A49i:** the first cut of the uptime join checked whether an hour's START INSTANT fell inside an evidenced interval, which is correct only when evidence happens to land exactly on the hour boundary — since heartbeats land a few minutes into an hour, this false-negatived every hour whose evidence wasn't at `:00`. Fixed to check [hourStart, hourEnd] OVERLAP against the interval instead; A49i is the regression trap. **Independent validation:** a smoke run of the real CLI against the live local `backtest_data` store (2026-07-28→31, read-only) reproduced the proposal's own §0/§0a hand-measured figures exactly — `capture begins 2026-07-29 10:43 UTC` and `longest gap 161.7s`, both independently derived by the new code from raw files rather than copied from the doc. **Tweaker: no new fence** — Part A adds no settings keys; HC27 already fences everything under `trade_store.`; HC28 stays free. **Display-string parity: NO OBLIGATION** — Part A's surfaces are console + a markdown file (the offline-report precedent, neither a parity surface); the marker write is silent, no snapshot line, no card binding, stated explicitly per the hard rule. **Reversibility:** Part A is additive and read-only — not running the verb *is* the rollback; the marker write is a few bytes appended once per process start and reads existing config, so removing the two new files reverts byte-identically. **Fixtures A49a–l** (A49m deferred to Session 2 alongside Part B): uptime parse across two process lives incl. the open trailing interval and a process closing without a DOWN line (a); the S1 join distinguishing before-app-started (expected-missing) from up-but-silent (defect) from up-and-clean (captured) (b); DEGRADED/REST evidence counted as "up" like OK, never conflated with down or defect (c); capture-era self-bounding from the store's first trade rather than `--from` (d); S3's exact-max-reported / breach-strictly-above-threshold arithmetic (e); S4 candle completeness short by an exact k (f); S1 gracefully skipped with a stated reason when both uptime sources are empty, S2–S4 still run (g); the `--strict` exit decision, defect vs expected-missing-only (h — pins the CLI's decision predicate; the literal process exit code is reasoned not harness-proven, `BacktestProgram.vb` stays out of this harness like every other verb); the two inversion traps this build exists to catch: analysis_log.csv rows alone resolving UP with ws_health.log silent, and a cross-GUID gap with NO evidence at all resolving DEFECT never expected-missing (i); S0's exact venue-diff enumeration against a stubbed response, zero HTTP (j); the not-capturing inversion trap — the SAME silence under marker OFF (zero defects) vs marker ON (defect) (k); unknown-scope firing unconditionally when no marker record exists at all, regardless of store state (l). Acceptance: solution + AutoTweaker + WhatIfRunner + CeilingAudit + BacktestRunner + OrderCheck build **0/0** Release; A1–A52a unregressed + A49a–l; verify-gate `prepush` GATE PASSED (run post-commit, per the standing note that it reads committed diffs). Local commit — trader tests before Session 2. |
...
Older entries are **not** deleted — **v59 down to v27** live verbatim in [`history-archive.md`](history-archive.md) §E, and v26 back to v0.33 below them.
