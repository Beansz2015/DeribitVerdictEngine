...
Current version: **v39**. Top-level blocks:
- `indicators` (per-indicator parameter blocks)
- `session_volume` (UTC bucket multipliers + per-session `execution_resolution` — v36)
- `resolution_profiles` (per-resolution ROC threshold overrides keyed by "1"/"3"/"5" — v36)
