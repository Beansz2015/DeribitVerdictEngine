...
|---|---|---|
| **Absorption `D-2` + `D-6d` Stage 1** (LIVE-STRIP CHANGE AND A DATASET BOUNDARY for `AbsorptionAggrUsd` / `AbsorptionRatio`; no scoring impact while `scoring_enabled` is false; settings-untouched) | 2026-09-24 | S1 of [`absorption-d2-stage1-rotation-build-spec.md`](absorption-d2-stage1-rotation-build-spec.md). `D-2`: pressing is episode-cumulative; the press queue, `PressQueueCap` and the window prune are removed, and `window_sec` is now unused. Identical below 10 s, larger above. Stage 1: shadow press on idle sides, seven close reasons (spec six plus `TouchCrossed`) and the `absorption_episodes.log` sidecar, one line per run. The live strip ratio moves; snapshot and cards do not. Fixtures `A88a`–`A88f`, `A89a`–`A89b` (spec ids `A78`/`A79` were taken). Harness 453 to 461. Packet: [`absorption-d2-s1-spec-back.md`](absorption-d2-s1-spec-back.md). NOT DEPLOYED — rides the single S2 deploy. Settings v68 untouched |
