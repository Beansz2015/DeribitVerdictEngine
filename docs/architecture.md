...
| Kelly sizing display-only | Sizing advisory only — no position management integration, zero scoring impact. **[v69]** p and b are measured from the live eval cache (`LivePerformanceTracker.ComputeKellyBook`): pooled per session, split into 3 terciles of the book's own placed net payoff, falling back to the session pool below the floor. Tiers are one class; the EST confidence-tier map is retired. The block renders whenever a Kelly side exists (`KellyHasSide`), in three states: below the `kelly.min_book_rows` floor, a compact `[NO EDGE]` at f* ≤ 0, or full sizing. It is hidden only on plain `NO TRADE` / `NO TRADE [TIE]`. Spec: [`kelly-one-class-placed-payoff-spec.md`](kelly-one-class-placed-payoff-spec.md). ⚠ This row read *"Suppressed when no edge (KellyF ≤ 0) … ATR-basis R:R … EST mode only"* until 2026-09-25; the suppression claim was false since `9becdbc` (2026-04-20), which gated on `KellyPWin`. Pre-v69 text: `git show 164447e:docs/architecture.md` |
