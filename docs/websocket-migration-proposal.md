...
11. **Early-resolution-on-confirmed-hit (live perf-strip latency)** — let a PENDING `LivePerformanceTracker` row resolve the instant a favourable/adverse barrier is *confirmed* hit in the bars available so far, instead of waiting for the full (resolution-scaled) window to elapse. Cuts the live-strip display lag for fast trades — most visible at 3-min Asia/London, where a winner at T+9 currently waits until T+45 to post (see `three-min-hold-window-recalibration-spec-back.md` §5). Display-only: the barrier-hit semantic resolves on the *first* chronological hit, so an early confirmed hit is final — the outcome never changes, only *when* it posts; zero scoring/CSV impact. **Caveat (must spec):** only safe with a "contiguous bars from T+3 up to the hit, no gap before it" guard — otherwise a Deribit gap before the observed hit could resolve early on an incomplete prefix (the window-complete check waits this out today). Lowest-value item in this list — pure polish on a glanceable surface; sequence it last, and build it against the streaming path (natural sibling of items 1/3) rather than retrofitting the REST-polled tracker.
