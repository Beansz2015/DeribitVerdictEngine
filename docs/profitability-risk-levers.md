...
|---|---|---|---|---|---|
| **L1** | Session size multipliers | Coarse per-session exposure (LONDON 0.5× etc.) | Order app — `session-policy-gate-proposal.md` §2 `size_mult` | PROPOSED (P1–P5) | Trader ticks P-table → order-app coordinator implements |
| **L2** | Tier/context subsets per session ("trade MEDIUM only", "CONFIRMED only") | Participation filtering with the counterfactual still measured (`refused: policy` dispositions) | Order app — same spec | PROPOSED (P1–P5) | Same handoff; first policy values at the live-ladder step (P5) |
...
| **L4** | Kelly CAL (EST→CAL) | Automates L1 from measured per-(tier×session×res) win rates; payload kelly fields become empirical | Engine — roadmap **W6-3** | Gated: M1–M5 matrix migration (the p-instrument) + eval-book depth per cell | Tick M1–M5; size the book-depth gate at pickup |
