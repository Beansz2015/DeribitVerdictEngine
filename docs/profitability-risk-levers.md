...
| **L4** | Kelly CAL (EST→CAL) | Automates L1 from measured per-(tier×session×res) win rates; payload kelly fields become empirical | Engine — roadmap **W6-3** | Matrix migration SHIPPED 07-21 (the p-instrument exists) — **BUT F1: the tier ladder is UNVERIFIED (STRONG n≈90, all CIs overlap); CAL would fit noise today. Gate: F4 fix shipped + n≥150 STRONG + ladder re-read via the offline report** | Wait for the F1 gate; then size the book-depth gate |
