...
**6. `TargetCapReason` moved to informational (coordinator correction 2026-07-23).** Original build included it as a scored one-hot on the "structure" reading of §2. Coordinator caught the failure: `TargetCapReason` is Step-5b OUTPUT (the bucket the placed-geometry arbitration emitted), not a scoring input that votes or modifies — and it's a mechanical proxy for target-distance class, so admitting it into X would let the challenger draft AUC off geometry difficulty the baseline effective score can't see, inflating the ΔAUC with a phantom "prize" that better combination could never actually harvest. Demoted to `InfoCategoricals`; univariate AUC reported alongside Absorption* in the informational side-column. A39e fixture updated to pin the absence explicitly (banned-name set now includes `TargetCapReason` on the column list, `ScoredCategoricalNames`, and `ScoredNumericNames`).

**7. `MTF15mTrend` / `MTF15mEMAAlignment` / `PriceVsEMA200` included.** Not listed by name in §2, but they are inputs the engine's MTF gate + regime alignment consume — they DO vote/modify. Included as SCORED one-hots because they meet the spec's rule ("everything that votes or modifies"). Coordinator confirmed this call.
