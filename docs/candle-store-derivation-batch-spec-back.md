...
**Code landed since this packet was written, and it is provably orthogonal — checked, not assumed.** The v64 trade-store build, the funding-backfill fix, the F1 capture-gate hoist, the candle-merge fix and the A51 tests together touched `Core/StoreFiles.vb`, `Core/TradeStoreWriter.vb`, `DeribitWsFeed.vb`, `TradeStoreGapRepair.vb`, `tools/BacktestRunner/HistoricalStore.vb`, two `.vbproj` files and `verify/ordercheck/Program.vb`. Diffed against the packet's base commit:

- **The three indicator files this batch's harness compiles — `Indicators_Volatility.vb`, `Indicators_Structure.vb`, `Indicators_Momentum.vb` — are unchanged**, as are `DynamicNorms.vb` and `ScoringEngine_Calculate_Scoring.vb`. So the re-run in §0 was a reproduction under **identical** indicator code, not merely a similar one.
- **No scoring path or render surface moved** — `ScoringEngine_Calculate_Verdict.vb`, `ScoringEngine_Helpers.vb`, `IndicatorResults.vb`, `MainForm_Analysis.vb`, `MainForm_PlaintextSnapshot.vb`, `MainForm_Render_Cards.vb` all untouched.
- **`settings.json` moved only in the new `trade_store` block** (plus `version` / `change_log`). **No indicator threshold changed** — the values this batch transcribed (`TTM.flat_threshold` 0.5, `OBV.trend_gate` 18.0, the `Volume` clamps, the session multipliers) are all still live.

The **`settings.local.json` overlay is spec-only — not built** — so nothing in it is in effect. Its one JOB 2-relevant consequence is folded into D-C.
...
- **The WS-vs-REST bar difference.** The matched replay disagrees on 1.65% / 0.78% of rows (1.41% / 0.78% on the repaired store). I attributed that to the known §12 *WS 3-min closed-bar volume undercount* row but **did not isolate it** — a real check would compare WS-built bars against REST bars directly, which needs a live capture window.
  ...
  **Sharpened at hand-over, and the direction matters.** The overlay review's §2.3 established that `ResolveSource` returns `_restSource` for a run whenever the feed is degraded and `ws_fallback_to_rest` is on — so **an unknown fraction of production rows were scored on REST candles, not WS ones.** My replay uses REST store bars, so those rows agree with it *trivially*. The measured disagreement is therefore a **blend**: `measured ≈ (WS-row share) × (true WS-vs-REST rate) + (REST-row share) × ~0`.

  **So 1.41% / 0.78% is a lower bound on the true WS-vs-REST bar disagreement, not an estimate of it** — and it cannot be corrected, because the degraded-fallback rate is itself unmeasurable: no CSV column records which source scored a row. This is the same gap the effective-source stamp would close (`overlay-d7-and-row-source-stamp-2026-07-31.md` §4). **Nothing in this packet's conclusions depends on that residual** — it is a caveat on one diagnostic figure, not on any recommendation — but a reviewer should not read 1.41% as "the WS/REST bars agree to 98.6%".
