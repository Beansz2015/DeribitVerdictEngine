...
>
> ✅ **MEASURED, weekday-scoped off `analysis_log_aws.csv` (840 absorption-active reads, 2026-09-01 15:50 → 09-09 14:39 UTC, ~6 weekday-days) — and it answers the *"unquantified, deliberately"* line above, which was true when written and stopped being true when `AbsorptionEpisodeSec` shipped:** p50 **2.15 s**, p75 **10.38 s**, mean **9.50 s**. **26.0 % of reads sit on an episode older than the window, and those reads carry 48.6 % of ALL logged `AbsorptionAggrUsd`** (442,070 of 909,850 USD). ⚠ **2026-09-13 (UTC) 8-weekday-day re-read: the 26 % holds (26.5 %, day-block 95 % 21.5–32.2 %); the 48.6 % does NOT — it is a lumpy sum (10 of 1,159 reads carry 56 % of all pressing), 36.4 % pooled, day-block 95 % 25.8–55.4 %. See [`absorption-episode-age-read-2026-09-13.md`](absorption-episode-age-read-2026-09-13.md).** **Span multiplier there: p50 2.25×, p75 3.80×, mean 3.01×.**
>
...
>
> ⛔⛔ **THE CONSEQUENCE FOR THIS PROPOSAL'S OWN §4.1 — AND THE FIRST VERSION OF THIS BOX GOT IT BACKWARDS, CORRECTED THE SAME DAY ON A MEASUREMENT.** ~~It claimed that on the median episode D-2 is arithmetically a no-op.~~ **MEASURED weekday-scoped off `analysis_log_aws.csv` (840 absorption-active reads, 2026-09-01 15:50 → 09-09 14:39 UTC): `AbsorptionEpisodeSec` p50 2.15 s but p75 10.38 s and MEAN 9.50 s — `D-2` binds on 26.0 % of reads, and those reads carry 48.6 % of ALL logged `AbsorptionAggrUsd`. Median span multiplier 2.25×. `D-2` IS NOT A NO-OP.** ⚠ **The 48.6 % is a 6-day point on a lumpy sum — the 2026-09-13 8-day re-read gives 36.4 %, day-block 95 % 25.8–55.4 %; the 26 % read share holds. [`absorption-episode-age-read-2026-09-13.md`](absorption-episode-age-read-2026-09-13.md).**
>
> ⭐⭐ **AND `D-2` IS A REPAIR, NOT A RISK.** **`PressSum` is WINDOW-scoped while `SizeStart − SizeMin` is EPISODE-scoped, so on those 26 % of reads `absorbRatio` already divides TEN SECONDS of pressing by a WHOLE EPISODE's depletion — it UNDERSTATES itself on exactly the episodes carrying half the pressing.** **Episode-cumulative puts both terms on one span.**
>
> ⛔ **ORDERING, and it is the opposite of the first draft's: `D-6d` Stage 2 shipped WITHOUT `D-2` makes `absorbRatio` FALL** — the denominator grows with episode length while the numerator saturates at the 10 s cap, so the fix reads as a regression. **Safe orders: `D-2` first, or both in one commit. The spec's `D-6d.3` now recommends (c) — `D-2` + the behaviour-neutral Stage 1 instrument together at the gate.**
>
...
| **D-2** | `window_sec` → **episode-cumulative pressing** (§4.1) | ✅ **Yes** — but **after D-1 reads**, so the effect is measured rather than projected. ⛔ **D-1 HAS SHIPPED (2026-09-01) AND ITS FIRST READING POINTS AGAINST §4.1 — median `EpisodeSec` 1.7 s, so episode-cumulative would SHORTEN the span, not extend it. See §4.1's 2026-09-03 box. PRELIMINARY: 2 weekdays of a ruled ~10. Do not build against this row before ~2026-09-15** ⛔⛔ **CORRECTED 2026-09-13 (UTC): the "would SHORTEN the span" reading in this cell is WRONG — refuted in §4.1's box, because the press queue is cleared at every episode open and close, so episode-cumulative is a SUPERSET and never shorter.** ⭐ **8-weekday-day read done 2026-09-13: `D-2` binds on 26.5 % of absorption-active reads (day-block 95 % 21.5–32.2 %) — [`absorption-episode-age-read-2026-09-13.md`](absorption-episode-age-read-2026-09-13.md). Final re-run from 2026-09-16 UTC, one command.** | ⚠ **Do not tick this as though the span were the whole problem.** The engine counts only **31 %** of the in-band flow its own 10 s window already admits (§4.3 box (b)) — a cumulative numerator accumulates over the same gap. **The span is the third-largest of three causes**, behind geometry and episode state |
