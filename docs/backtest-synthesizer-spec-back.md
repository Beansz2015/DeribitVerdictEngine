...
**Clearance — RULED 2026-07-31 (Fable seat): PARTIAL.** §7.5's conditional clearance withheld the VWAP-sensitive study class "until the anchor fix lands and re-validates." It landed and re-validated; the window is exact, the values are not. The ruling: **window-consuming studies CLEARED · VWAP values CLEARED for coarse/structural use against a documented noise floor · fine dev-threshold sweeps (< 5 bps steps) WITHHELD pending the D2 accumulator-tolerance reclass · funding unchanged** (`FundingMomentum` 22.02 %, structurally approximate, discouraged — the anchor is not on the funding path).

⚠ **RESOLVED 2026-07-30 by measurement: the residual is a VOLUME-INPUT difference, not a tolerance.** H/L are byte-identical (Donchian 100.00 %, max |Δ| 0), closes agree 99.5 %+ (EMA family), so Volume is the only VWAP input that can move it — and conditioning the 840 buckets on volume agreement gives **100.0 % VWAP match (199/199, median |Δ| $0.00)** where volume agrees vs 42.6 % where it differs. The bps-scale reclass should **not** be specced. The fine-sweep condition is gated on **volume fidelity**, i.e. the D3 forming-bar lane, not on a tolerance task. Evidence: `pre-aug1-batch-spec-back.md` §7.2b. **Window + coarse/structural clearances unaffected.**

⚠ **SUPERSEDED — D1 RE-RULED 2026-07-31 (incoming orchestrator seat): FULL CLEARANCE on the VWAP axis.** The volume difference above was a **~64,000× unit error** in `ReplayLoop.BuildFormingStub` (`TradeRecord.Amount`, USD notional, summed into `Candle.Volume`, BTC), fixed in `ae8a1f6` with fixture A47b. Post-fix on the same 840 rows: **VWAP 100.00 %**, σ bands **99.76–100.00 %**, OBVTrend/OBVDiv **99.76 / 99.52 %**, verdict/tier **79.64 / 86.19 %**. Residual 1 above ("the VWAP values did not reach EMA-class") is therefore **closed** — they did reach it, once the input was right; and residual 2 (OBV unmoved) is closed with it. The `~1.3 bps` noise floor and the whole tolerance-class question are **withdrawn** — internal consistency could not detect a unit error, which is the A43f lesson repeating.

**Now cleared:** window-consuming studies · VWAP values outright (faithful band) · fine dev-threshold sweeps < 5 bps. **Still NOT cleared:** `VolumeRatio` at **65.00 %** stays *advisory only* — that residual is genuine forming-bar partiality (the D3 lane), not the unit bug, because `VolumeRatio` **is** the partial terminal bar whereas VWAP merely contains it among 240; funding unchanged at 22.02 % and still discouraged; ATR/ADX/RSI unchanged and still *do not use*. Canonical record: [`backtest-overlap-validation-2026-07-30.md`](backtest-overlap-validation-2026-07-30.md) §10.4.
