...
⚠ **The noise-floor figure is disputed, so the fine-sweep condition is unresolved** — the ruling cites mean |Δ| $8.6 (≈1.3 bps) where the run published **69.79** (≈10.9 bps) over a bimodal distribution (p50 $1.26 / p75 $101.53 / p90 $235.53), and the "session-edge tail" reading tests negative against `VWAPSessionCandles`. Evidence + four eliminated hypotheses: `pre-aug1-batch-spec-back.md` §7. **The window and coarse/structural clearances are unaffected** — they rest on the window being exact, which is not in dispute.
