...
| `BELOW_MIN_MOVE` (v35) | The directional verdict's realistic move (post-cap, post-Step-5b) is smaller than the minimum tradeable move. **[v62]** That floor is now *composed* — `scoring.trade_costs`: round-trip execution cost for the assumed style plus your minimum acceptable **net** move (defaults 2 × 1.5 bps maker + 0.05% = 0.08% of price, ≈$50 at $62k — the same number the retired flat `min_tradeable_move_pct` carried). Overrides the verdict to `NO TRADE` even though scores/levels/breakdown still render for diagnostics. Computed at the **end** of `Calculate()`, after the regime veto and MTF gate — same pattern as those vetoes. Most common on low-ATR Asia/London runs by design. |
...
- **`NEUTRAL`**: no score.
- **Step 5b POC target tier** (separate from scoring; `SignalEmitter.ComputeStructuralSideLevels`, legacy `enabled:false` twin in `ScoringEngine_Calculate_Verdict.vb`): the **long** POC tier opens on `NEAR_HVN_SUPPORT` or `IN_LVN_BEAR` — both labels put the POC ABOVE price. The **short** POC tier opens on `NEAR_HVN_RESIST` or `IN_LVN_BULL` — both put the POC BELOW price. An open tier places the target at the POC when the POC sits on the target side within `structural_levels.target_max_atr_mult × ATR` (legacy path: closer than the raw ATR target), after the swing and nearest-HVN tiers. ⚠ An `IN_LVN_*` state does not reach the POC tier in practice: price sits outside the POC bucket, so the POC bucket is itself the nearest HVN on that side and the HVN tier places first. ⚠ Until the 2026-09-24 (UTC) `D-1` fix the two `NEAR_HVN_*` labels were swapped in this gate, so the tier never placed on them (`docs/engine-fix-build-spec-2026-09-21.md` §3).

**Display colour:** green for support / LVN-bull, red for resist / LVN-bear, dim for neutral. `HVN@POC` shows `YES` / `NO`.

**Interpretation:**

- `NEAR_HVN_RESIST` means price sits AT or just ABOVE the POC, inside the `hvnProximityPct` band (the classification table above). The POC — the high-volume node — is just BELOW price. ⚠ The name reads as "resistance above", but the geometry is price above the node. The engine turns it into a short vote (price near the POC from above; see **Scoring use**) and opens the SHORT target's POC tier, because the POC is below price. `NEAR_HVN_SUPPORT` is the mirror: price just below the POC, a long vote, and the LONG target's POC tier. *(Corrected 2026-09-24 (UTC) under ruling `D-2` of `docs/medium-tier-bug-hunt-spec-back.md`; this line previously described the opposite geometry.)*
...
- **Off-surface prefixes** — `scoring.trade_costs.` (v62: the whole fee model + the min-net risk preference behind the min-move floor — HARD CONSTRAINT 26; the retired flat `scoring.min_tradeable_move_pct` no longer resolves at all)
- **Off-surface exact keys** — `indicators.ofi.averaging_enabled`, `indicators.aggressor_velocity.enabled` / `.scoring_enabled`, `scoring.structural_levels.enabled` / `.stop_too_loose_mode` (feature switches, not thresholds)
...
- **Min net move % (after fees)** — `trade_costs.min_net_move_pct` (0.0005). **[v62]** The smallest take-profit distance a directional call must clear on its **placed** target, measured *after* execution costs. The gate's actual floor is this plus the round-trip fee (`0.0003` at the default maker/maker 1.5 bps), i.e. `0.0008` — so sweeping this knob sweeps the whole floor, one-for-one. The fee keys themselves are deliberately not sweepable. *Seen in:* Verdict flipping to `NO TRADE`, with Context `BELOW_MIN_MOVE`.
...
- **Two knobs together (2-D grid).** `0.45:0.60:0.05` in **Verdict MED %** and `0.0003:0.0009:0.0002` in **Min net move % (after fees)** → 16 cells; watch the population-shift line.
