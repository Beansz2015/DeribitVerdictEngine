...
| verify-gate | `prepush` mode — **GATE PASSED**, exit 0. `display-parity`: no snapshot/card drift. `version-bump`: no engine-path change (`analysis/` is under the engine-path glob but the eval semantics is a measurement layer, not scoring — per the spec constraint the change_log stays untouched and the §15 row calls out "no settings bump"). |
