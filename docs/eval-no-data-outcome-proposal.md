...
**Date:** 2026-07-21 · **Status:** ✅ **SHIPPED 2026-07-21 — `75a2694`** *"fix(eval): F4 NO_DATA outcome + v5→v6 sweep; RoundStatsBuilder on placed geometry; WhatIfRunner in gate"*. N1–N5 all ticked and all built; the successor display pass (F2/F3/F12, `4eef0d8`, v55) shipped after it per N5's sequencing. **Verified live 2026-08-01:** `analysis_eval_cache.csv` carries `# schema=v6`, `analysis_eval_cache.csv.v5.bak` exists (2026-07-21 21:41), and the cache holds **120 `NO_DATA` rows** — the sweep ran and the fix is operating. · **Priority: FIRST of the F-series** *(historic — it was)*

> **Header history, kept because it caused a real error.** This line read *"PROPOSED — D-table awaits trader"* from the tick until 2026-08-01, i.e. it was stale while the work was **ticked, built, shipped and live**. On 2026-08-01 the orchestrator seat corrected it to "BUILD-AUTHORIZED" by reading §3's tick line — a half-correction that fixed the wrong half and left it looking like available work. It was carried on the trader tick queue as an outstanding item, ticked by the trader, and an implementer conversation was recommended for it. The error surfaced only when reading the code to write that implementer brief. See [`trader-tick-queue.md`](trader-tick-queue.md) §2a for the convention this produced. (correctness; every measurement inherits it — F1's re-read, W6 audits, Kelly CAL inputs).
