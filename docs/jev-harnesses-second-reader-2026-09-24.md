...
|---|---|---|---|
| `SR-D1` | `FP-Q1` item-level gate (`SR-4`) | (a) refuse the run when any scope candidate lacks a seat line, with `-AllowUnbaselinedItems` as the opt-out — the `FP-D24` pattern · (b) judge only the covered candidates, count the rest as skipped | **(a).** It guarantees more and matches the item gates in all four other harnesses. (b) silently shrinks the population, the class `harness-shadow-mode-protocol.md` §4e names — ✅ **DONE 2026-09-24 (UTC).** Built as `FP-D32` in `tools/checks/fixture-parser.ps1` (revision 5): the file-level `SCOPE_BASELINE_STATE=SUPPLIED` check is unchanged, but every `needsJevParams` item absent from a supplied, found baseline now stops the WHOLE scope-classification step cold (zero `FP-Q1` calls) and the run exits 2 `EXIT_REASON=BASELINE_INCOMPLETE`/`SCOPE_UNBASELINED_ITEMS=`, printed after coverage so the "prints on every exit path" invariant holds. `-AllowUnbaselinedItems` opts out (shared with `FP-1`/`FP-2`'s own gate, not a second flag). Proved with a synthetic input and the transport seam, no key: `tools/checks/selftest/fixture-parser-selftest.ps1` checks E1 (refusal, zero calls, names the missing param) and E2 (opt-out, 310 scope calls = 62 candidates × 5 samples). |
| `SR-D2` | `Q29`'s no-answer check was never run (`SR-2`). Its section Nouls were judged, so it is in the ledger | (a) run the no-answer check alone once (a `-NoAnswerOnly` mode; about 150 calls to rebuild the re-ranked top 5 at `-Samples 5`, or 1 call over the BM25 top 5) · (b) leave it; the trial meets new no-answer queries | **(a), over the re-ranked top 5.** The seat has not seen any no-answer output for `Q29`, so it is still a clean measurement of the one trap query in the set. It needs a ledger exception, which is why it is reserved — ✅ **DONE 2026-09-24 (UTC).** Built `-NoAnswerOnly <id>` (Mode 3) in `tools/checks/doc-reranker.ps1`, gated by `-AllowSpentOnce <id> -Reason "..."` (refuses `RESERVED_QUERY_SPENT` without it, refuses `ALLOW_SPENT_ONCE_UNUSED` if passed needlessly) and logged to a new `exceptions` array in `docs/harness-runs/doc-reranker-spent-queries.json` (the existing `spent` entry for `Q29` is untouched, never duplicated). Run once, keyed, at `-Rev af5d6af -Samples 5`: `noul=0.07` — **no doc answers it**, matching the expected trap answer (`trader-profile.md` §4 records only the MACD rejection). 151 calls, 130,313 input tokens. Full result: `docs/harness-runs/doc-reranker-reserved-run-2026-09-24.md` §3. Self-test coverage: `tools/checks/selftest/doc-reranker-selftest.ps1` checks G/H/I/J. |
...
| `SR-D6` | `FP-2` relies on the title (§4) | (a) add a title-stripped `FP-2` arm beside the current one, the `FP-1v2` pattern, measured at the next fixture review · (b) strip titles from `FP-2`'s state · (c) note only | **(a).** It keeps comparability and measures the dependence on real names. (b) changes a measured detector without a re-measure — ✅ **DONE 2026-09-24 (UTC).** Built as `FP-D33` in `tools/checks/fixture-parser.ps1` (revision 5): `FP-2t` asks the same question over the same Sub body beside `FP-2`, in the same call/loop, with every `Check("...")` title replaced by the neutral placeholder `"check"`. `FP-2` alone still drives `FP2_BAD_VERDICTS`/the exit code; `FP-2t` reports beside it (`FP2T_ITEMS_SAME_PLURALITY_AS_FP2`, `FP2T_WAF_BLOCKED`), the `FP-1v2` footing. Acceptance run (the same 6 mutation fixtures, titles renamed, 5 samples each side, via the existing `-RenameTitles` driver — no new flag needed, `FP-2t` rides along automatically): `FP-2` 5 of 6 flagged (unchanged from the 2026-09-24 title-rename run); `FP-2t` also 5 of 6, the SAME five, and does **NOT** flag `A23a` either (`name_matches`, but `UNSTABLE` 3 of 5 at mean top prob 0.404, its weakest row) — the body's own code evidence is genuinely weak here, not purely a title artefact. Full result: `docs/harness-runs/fixture-parser-fp2t-mutation-2026-09-24.md`. |
