...
|---|---|---|
| **D1** | Both boxes capture, or AWS only? | **AWS ONLY — ruled against the recommendation, on a better reason than the one I offered.** I argued for redundancy; the trader's ground is that **the end goal is the app running on AWS and not on the local box at all**, so dual capture builds for a topology the project is leaving. That supersedes the redundancy argument: redundancy across a box you intend to retire is not redundancy, it is migration debt. **Consequences, which the build must honour — see §7.1.** ⚠ **AMENDED 2026-08-07 — see D1-a below. The end-state ruling STANDS; only the interim changed.** |
| **D1-a** | Interim only: does the local box capture while both boxes exist? | ⚠⚠ **PREMISE WITHDRAWN 2026-08-08 — this row awaits a trader re-read.** The measurement below is retracted: the two boxes disagree on *amounts at shared timestamps*, not on which trades exist (zero timestamps were absent from AWS), and the root cause is that the store carries **no `trade_id`**, so no whole-row comparison between two books means anything. Full correction in `aws-collector-deploy-checklist.md` §4a. **Recommendation: revert local capture to OFF until `trade_id` ships** — two books cannot be merged, tape written meanwhile is permanently unmergeable, and D1's end state was never in dispute. *The original text follows, retained per the convention that a retired claim is quoted and labelled rather than deleted.* ⚠ **YES — TEMPORARY, ruled 2026-08-07 (trader), on measurement.** The first store copy-back showed the sole capturing box at **78.8 %** complete over 2026-08-03 21:44 → 08-05 13:13 UTC, against **98.5 %** on a second box, with AWS **healthy throughout** — 16,459 trades existed only on the local box. Loss is scattered across 36 hours, so it is per-connection delivery difference, not an outage, and **no single subscriber can detect it about itself** (`docs/aws-collector-deploy-checklist.md` §4a). **D1's original ground is untouched and still correct** — the end state is AWS-only. What D1 did not price is a sole capturer being *systematically incomplete* while healthy. **So local capture returns as a second sampler, for data collection only, and retires when the local box does.** ⚠ **`bin\Debug` ONLY — corrected 2026-08-07 the same day, before it shipped.** `bin\Release` keeps its overlay and must not capture: Release is the AWS deploy source, so a store inside it gets xcopied over AWS's own tape. Measured that day: AWS's August file held 228,163 rows against the local Release file's 78,798, so the copy would have destroyed about 150,000 trades that cannot be refetched. Hazard box and pre-flight: `aws-collector-deploy-checklist.md` §1a and §1b. ⚠ **Known bias, trader-raised and it binds any tape analysis:** the local box runs when the trader is at the desk, which is active hours and mostly NY. So **tape completeness will be higher for NY than for ASIA or LONDON.** The CSV book is nearly unaffected — AWS-preferred minute-key dedup discards about 99.9 % of the local box's overlapping rows — but **the tape store is merged as a plain union with no preference**, so the bias lands there in full. **No tape-derived measure may compare across sessions without accounting for uneven completeness.** Same family as the J-B scoping rule: do not read a number off a book whose coverage varies with the thing being measured |
