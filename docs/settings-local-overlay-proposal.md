...
| **D7** | A REST-mode box is a legitimate need that §2.2 now blocks. How should it be expressed? | (a) as a tracked `settings.json` change with a version bump — deliberate and version-visible · (b) admit `network.transport` to the overlay anyway, accepting the straddle · (c) admit it AND exclude that box's rows from the pooled book | **(a) for now.** The review's point stands: if a box needs REST, that should be a deliberate, version-visible act rather than a silent overlay. (c) is the honest version of (b) but needs a pooling-side rule that does not exist yet, and inventing one here would be scope creep. Raised as a decision rather than buried, because §2.2 removes a capability the first draft offered. |
...
**New decision surfaced:** D7 — §2.2 removes the REST-box capability the first draft offered, so how a REST box should be expressed is now an open question rather than an assumed answer.
