...
**Date:** 2026-07-27 (Fable coordinator). **Status:** PROPOSED — D1–D6 await trader tick; build-ready on tick (Opus, medium effort).
...
1. **Eval net-EV rider** (analysis-only, no ⚠): what-if EV-in-ATR + any report EV column subtract per-row round-trip cost (`fee_pct × price / ATR`; loss side may add the maker→taker emergency delta). Slot: post-Aug-1, bundled with the EV ± dispersion report column discussed 2026-07-27 (the Sharpe-question resolution — metric follows cost model).
