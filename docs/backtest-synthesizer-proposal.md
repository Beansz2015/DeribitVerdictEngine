...
**7.5 Second live-code finding + CONDITIONAL CLEARANCE (coordinator, 2026-07-31).** The §7.1 re-validation (verdict agreement 71→74%, EMA/Donchian/Regime columns recovered to 97–100%) exposed the one impurity in the "pure core" premise: `Indicators_Volatility.vb::GetSessionCandles` reads `DateTime.UtcNow` for the VWAP session anchor — correct live (now ≈ candle time), wrong under replay (anchors historical ticks to the real clock; VWAP family match collapsed to ~44%, row-proof in the validation report §8.6). **Amendment: ONE micro engine change is authorized for spec** — parameterize the anchor (`Optional nowUtc As DateTime? = Nothing`, defaulting to UtcNow ⇒ live byte-identical; replay passes the bar close), fixture-pinned for default-path identity. This is the sole exception to §0's zero-engine-changes claim, it changes zero live behavior, and it ships as its own micro-commit with the standard gate. — **Clearance ruling:** the synthesizer is **CLEARED for geometry/placement-class studies now** (the target ladder never reads VWAP; population caveats per §2 apply as always) and **NOT cleared for VWAP- or funding-sensitive indicator studies** until the anchor fix lands and re-validates (funding stays structurally approximate — the coarse-history endpoint is the sole cause of the 22% momentum agreement, documented, not fixable free). Residual ATR/ADX-class partial matches trace to stub-latency variance under brutally tight tolerances, not assembly error — acceptable, disclosed.
